...
if($tipo_formato=="ARCHIVO EXCEL"){
    ...
    while ($linea = pg_fetch_array($resultado, null, PGSQL_ASSOC)) {  $campos=""; $registro=""; $i=0;
	    if(($inc_encab=="SI")and($l==0)){ $i=0;
		   ?><tr><?php
		   foreach ($linea as $valor_col){   ?> <td width="150"><?php echo pg_field_name($resultado,$i); ?></td> <?php	$i=$i+1; } 
		   ?></tr> <?php
		}
		?><tr><?php
		foreach ($linea as $valor_col){   ?> <td width="150"><?php echo $valor_col; ?></td> <?php	 } 
		?></tr> <?php
		$l=$l+1;
    }
