<?php include ("../class/conect.php"); include ("../class/funciones.php"); $login=$_GET["usuario"]; $cod_presup=$_GET["codigo"]; $fuente_financ=$_GET["fuente"]; $url="Det_asig_part.php?usuario=".$login;
...
if (pg_ErrorMessage($conn)) { ?> <script language="JavaScript"> muestra('OCURRIO UN ERROR CONECTANDO LA BASE DE DATOS'); </script> <?php }
  ...
  if ($filas==0){?> <script language="JavaScript"> muestra('CODIGO NO ASIGNADO AL USUARIO'); </script>  <?php }
