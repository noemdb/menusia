...
function cerrar_catalogo(mcodigo,mnombre){
  ...
  window.opener.document.forms[0].txtdenominacion.value = mnombre;
...
<?php       $criterio=""; $txt_criterio=""; $pagina=1;$inicio=1;$final=1; $numPags=1;
        ...
        $criterio = " where (length(codigo_presup)=".$long_c.") and (codigo_presup like '%" . $txt_criterio . "%' or nombre like '%" . $txt_criterio . "%')"; } }
         $sql="SELECT * FROM ingre001 ".$criterio;   $res=pg_query($sql); $numeroRegistros=pg_num_rows($res);
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron resultados</font>";
        }else{
               if ($_GET["orden"]!=""){$orden=$_GET["orden"];}else{$orden="codigo_presup";} $tamPag=10;				
                ...
                echo "<th height='20' bgcolor='#99CCFF' ><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=codigo_presup&criterio=".$txt_criterio."'>C�digo Ingreso</a></th>";
                echo "<th height='20' bgcolor='#99CCFF'><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=nombre&criterio=".$txt_criterio."'>Denominaci�n</a></th>";
                $linea=0; $Salir=false;
...
<form action="Cat_codigo_ing.php" method="get">
