...
function Llamar_cat(){var murl;
  murl= "Cat_cuentas_asoc.php?criterio="+document.form1.criterio.value;
  LlamarURL(murl);}
function cerrar_catalogo(mcuenta,mnombre){
  window.opener.document.forms[0].txtCta_Asociada.value=mcuenta;
  window.opener.document.forms[0].txtNombre_Asociada.value=mnombre;
  window.opener.document.forms[0].txtCta_Asociada.focus();
  window.close();
}
...
<html>
<head>
<title>SIPAP CONTABILIDAD FINANCIERA (Catalogo de Cuentas)</title>
<meta http-equiv="Content-type" content="text/html;charset=UTF-8">
<meta http-equiv="Pragma" content="no-cache" />
<LINK href="../class/sia.css" type="text/css" rel="stylesheet">
</head><body>
<?php 
        $criterio="where (cargable='C')"; $txt_criterio=""; $pagina=1;$inicio=1;$final=1;
        if ($_GET){if ($_GET["criterio"]!=""){$txt_criterio=$_GET["criterio"];$txt_criterio=strtoupper ($txt_criterio);
        $criterio=" where (codigo_cuenta like '%" . $txt_criterio . "%' or nombre_cuenta like '%" . $txt_criterio . "%') And (Cargable='C')";
...
<form  name="form1" action="Cat_cuentas_asoc.php" method="get" onSubmit="return revisar()">
<table border="0">
  <tr>
	<td>
