...
$rif_emp="G-20000831-8"; $flete="NUESTRO CARGO";   $direccion_emp="CALLEJON LA MOSCA ENTRE AV. CEDE�O Y YARACUY";
...
$sql="SELECT * FROM art_ord_compra where nro_orden='$nro_orden' and tipo_compromiso='$tipo_compromiso' order by nro_linea"; 
$sql="SELECT * FROM art_ord_compra where nro_orden='$nro_orden' and tipo_compromiso='$tipo_compromiso' order by codigo_articulo,nro_linea"; 
$res=pg_query($sql); $filas=pg_num_rows($res); $cant_tot_art=$filas; $pag_act=1; $pag_hasta=1; $max_art_pag=25; 

$nro_art_ant=0; $sub_total=0; $total_impuesto=0; $total_orden=0; $tasa_impuesto=0; $sub_total_iva=0; $total_descuento=0;
...
class PDF extends FPDF{ 
	function Header(){global $nro_orden; global $des_unidad_sol; global $fecha_orden; global $nombre; global $rif_proveedor; 
	   ...
	   global $nil; global $sunacoop; global $forma_pago; global $telefono; global $direccion_entrega; global $func_inv; global $nro_requisicion;	
		...
		$this->SetFont('Arial','B',7);
		$this->Cell(30,3,'TELEFONO',1,0,'C',true);
		$this->Cell(25,3,'FORMA DE PAGO',1,0,'C',true);
		$this->Cell(145,3,'UNIDAD SOLICITANTE',1,1,'C',true);
		$this->SetFont('Arial','',7);
		$this->Cell(30,4,$telefono,1,0,'C');
		$this->SetFont('Arial','',8);
		$this->Cell(25,4,$forma_pago,1,0,'C');
		$this->Cell(145,4,$des_unidad_sol,1,1,'C');
		$this->SetFont('Arial','B',7);
		...
		$this->Cell(25,4,$nro_requisicion,1,1,'C');
		$this->MultiCell(200,3,'POR CONCEPTO DE: '.$concepto,0);
		$this->Cell(200,2,' ',0,1,'C');		
		$y=$this->GetY(); $x=$y;		
		$this->Line(10,$y-0.2,210,$y-0.2);		
		$this->SetFont('Arial','',7);
		$this->Cell(25,3,'PARTIDA',0,0,'C',true);		
		$this->Cell(100,3,' ',0,0,'C',true);
		$this->Cell(20,3,'UNIDAD DE',0,0,'C',true);
		$this->Cell(10,3,'CANT.',0,0,'C',true);
		
		$this->Cell(15,3,'PRECIO',0,0,'C',true);
		$this->Cell(15,3,'MONTO',0,0,'C',true);
		$this->Cell(14.8,3,' ',0,1,'C',true);	


		
		$this->Cell(25,3,'PRESUPUESTARIA',0,0,'C',true);		
		$this->Cell(100,3,'DESCRIPCION DEL ARTICULO',0,0,'C',true);
		
		$this->Cell(20,3,'MEDIDA',0,0,'C',true);
		$this->Cell(10,3,'SOLIC.',0,0,'C',true);
		$this->Cell(15,3,'UNITARIO',0,0,'C',true);
		$this->Cell(15,3,'DESC.',0,0,'C',true);
		$this->Cell(14.8,3,'TOTAL',0,1,'C',true);
		$y=$this->GetY();		
		$this->Line(10,$y+0.1,210,$y+0.1);
		$this->Line(10,$x-0.1,10,189);
		
        $this->Line(35,$x,35,189);	
        $this->Line(135,$x,135,201);	
        $this->Line(155,$x,155,189);
		$this->Line(165,$x,165,189);
        $this->Line(180,$x,180,189);
        $this->Line(195,$x,195,189);		
    }
	function Footer(){ global $sub_total; global $total_impuesto; global $total_orden; global $tasa_impuesto;
		...
		$this->Cell(140,4,'SUB-TOTAL BS.',0,0,'R');			
		$this->Cell(19.8,4,$stotal,0,1,'R');
        if($imp_t=="S"){	
		$part1=$monto_letras; $l=strlen($part1); if($l>70){$part1=substr($monto_letras,0,70); $part2=substr($monto_letras,71,70); }
        $this->Cell(125,4,$part1,0,0);        		
		$this->Cell(55,4,'IVA '.$tasa_impuesto."% SOBRE ". $itotal,0,0,'R');
        ...
        $this->Cell(40,3,'PRESIDENTE',0,1,'C');		
		$x=$this->GetY()+0.5;
		$this->Line(50,$z,50,$x);	
		$this->Line(90,$z,90,$x);	
  ...
  $i=0; $prev_cod_art=""; $prev_cod_part=""; $cant_articulo=0; $total_articulo=0; $descuento=0;
  $res=pg_query($sql);$filas=pg_num_rows($res);
  while($registro=pg_fetch_array($res)){ $i=$i+1; if($prev_cod_art==""){$prev_cod_art=$registro["codigo_articulo"];}	
	
	if($prev_cod_art<>$registro["codigo_articulo"]){	
		$c_articulo=formato_monto($cant_articulo); $costo=formato_monto($costo); $t_articulo=formato_monto($total_articulo); $m_descuento=formato_monto($descuento);
		if($php_os=="WINNT"){$denominacion=$denominacion;}else{$denominacion=utf8_decode($denominacion);}
		if($tasa_impuesto>0){$denominacion=$denominacion."*"; $sub_total_iva=$sub_total_iva+$total_articulo;}
		$pdf->Cell(25,3,$cod_partida,0,0,'C');
		$x=$pdf->GetX();   $y=$pdf->GetY();  $w=100;		   
		$pdf->SetXY($x+$w,$y);	
		$pdf->Cell(20,3,$unidad_medida,0,0,'C');
		$pdf->Cell(10,3,$cant_articulo,0,0,'R');
		$pdf->Cell(15,3,$costo,0,0,'R');
		$pdf->Cell(15,3,$m_descuento,0,0,'R');
		$pdf->Cell(15,3,$t_articulo,0,0,'R'); 
		$pdf->SetXY($x,$y);	
		$pdf->MultiCell($w,3,$denominacion,0); 	
	    $cant_articulo=0; $total_articulo=0; $descuento=0; $prev_cod_art=$registro["codigo_articulo"];
	}
	
	$denominacion=$registro["descripcion_articulo"]; $unidad_medida=$registro["unidad_medida"]; 
	$codigo_articulo=$registro["codigo_articulo"]; $cod_partida=$registro["cod_partida"];
	$sub_total=$sub_total+$registro["total_articulo"];    $cant_articulo=$cant_articulo+$registro["cantidad_ordenada"];
	$costo=$registro["costo"]; $tasa_impuesto=$registro["tasa_impuesto"]; $total_articulo=$total_articulo+$registro["total_articulo"];
	$total_descuento=$total_descuento+$registro["monto_descuento"]; $descuento=$descuento+$registro["monto_descuento"];	
	$imp_art=$cant_articulo*$costo*($tasa_impuesto/100); $total_impuesto=$total_impuesto+$imp_art;	
  }
  if($i>0){
    $c_articulo=formato_monto($cant_articulo); $costo=formato_monto($costo); $t_articulo=formato_monto($total_articulo); $m_descuento=formato_monto($descuento);
	if($php_os=="WINNT"){$denominacion=$denominacion;}else{$denominacion=utf8_decode($denominacion);}
