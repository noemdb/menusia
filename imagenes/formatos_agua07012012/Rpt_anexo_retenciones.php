...
class PDF extends FPDF{ 
	function Header(){global $nro_orden; global $tipo_orden; global $fecha; global $php_os; global $total_ret; 
  ...
  while(($registro=pg_fetch_array($res))and($error==0)){
   ...
   $monto=formato_monto($monto);
    $pdf->Cell(13,3,$registro["tipo_retencion"],0,0,'L'); 
	$pdf->Cell(110,3,$registro["descripcion_ret"],0,0,'L'); 
	$pdf->Cell(37,3,$registro["cod_presup_ret"],0,0,'L');
    $pdf->Cell(12,3,$tasa,0,0,'R'); 		  
	$pdf->Cell(28,3,$monto,0,1,'R');
