...
function cerrar_catalogo(codigo_departamento,des_departam){
  window.opener.document.forms[0].txtcodigo_departamento_h.value = codigo_departamento;
  window.opener.document.forms[0].txtdescripcion_dep_h.value = des_departam;
  window.opener.document.forms[0].txtcodigo_departamento_h.focus();
  window.close();
}
</script>
...
<html>
<head>
<title>SIPAP N&Oacute;MINA Y PERSONAL (Catalogo de Departamentos)</title>
...
<?
        ...
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron resultados</font>"; $pagina=1; $inicio=1; $final=1; $numPags=1;
              ...
              if ($_GET["orden"]==""){$orden="codigo_departamento";}else{$orden=$_GET["orden"];} $tamPag=10;if ($_GET["pagina"]==""){$pagina=1;$inicio=1;$final=$tamPag;}else{$pagina=$_GET["pagina"];}$limitInf=($pagina-1)*$tamPag;$numPags=ceil($numeroRegistros/$tamPag);
                ...
                $sql="SELECT * FROM nom005 ".$criterio." ORDER BY ".$orden;
                $res=pg_query($sql);
