...
function cerrar_catalogo(cedula){
  window.opener.document.forms[0].txtcedula_d.value = cedula;
...
<?
        ...
        $criterio = " where cedula like '%" . $txt_criterio . "%'";}}
        ...
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron resultados</font>"; $pagina=1;$inicio=1;$final=1; $numPags=1; 
              ...
              if ($_GET["orden"]==""){$orden="cedula";}else{$orden=$_GET["orden"];} $tamPag=10;if ($_GET["pagina"]==""){$pagina=1;$inicio=1;$final=$tamPag;}else{$pagina=$_GET["pagina"];}$limitInf=($pagina-1)*$tamPag;$numPags=ceil($numeroRegistros/$tamPag);
                ...
                echo "<th bgcolor='#00CCFF' ><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=cedula&criterio=".$txt_criterio."'>C&eacute;dula</a></th>";
                $linea=0;   $Salir=false;
...
?>
<!-- tabla de resultados -->
  <tr bgcolor='#FFFFFF' bordercolor='#000000' onMouseOver="this.style.backgroundColor='#CCCCCC';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#FFFFFF'"o"];" onDblClick="javascript:cerrar_catalogo('<? echo $registro["cedula"]; ?>')" >
...
<form action="../Nomina/Cat_cedulad.php" method="get">
