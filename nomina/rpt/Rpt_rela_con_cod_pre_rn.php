<? include ("../../class/seguridad.inc"); include ("../../class/conects.php"); include("../../class/fun_fechas.php"); include("../../class/fun_numeros.php");   include ("../../class/configura.inc"); error_reporting(E_ALL ^ E_NOTICE); include ("../../class/phpreports/PHPReportMaker.php");
   $tipo_nomina_d=$_GET["tipo_nomina_d"]; $tipo_nomina_h=$_GET["tipo_nomina_h"]; $cod_conceptod=$_GET["cod_conceptod"]; $cod_conceptoh=$_GET["cod_conceptoh"]; $act_hist=$_GET["act_hist"]; $fecha_nom=$_GET["fecha_nom"];
   $forma_pago=$_GET["forma_pago"]; $cod_presup_d=$_GET["cod_presup_d"];$cod_presup_h=$_GET["cod_presup_h"]; $tipo_concepto=$_GET["tipo_concepto"]; $num_periodos=$_GET["num_periodos"];
   $estatus_trab_d=$_GET["estatus_trab_d"]; $tipo_calculo=$_GET["tipo_calculo"]; $detalle_concepto=$_GET["detalle_concepto"];$detalle_trabajador=$_GET["detalle_trabajador"];$detalle_aporte=$_GET["detalle_aporte"]; $tipo_rpt=$_GET["tipo_rpt"];   $php_os=PHP_OS;
   ...
   $Sql="";$date = date("d-m-Y"); $hora = date("h:i:s a"); $cfechan=formato_aaaammdd($fecha_nom);  $criterio2="Nomina";   $criterio3="FECHA AL ".$fecha_nom;
   $criterio="rpt_nom_cal WHERE (oculto='NO') and (cod_concepto<>'VVV')  "; $num_rpt=1;
   if($act_hist=='S'){$criterio="rpt_nom_hist WHERE (fecha_p_hasta='".$cfechan."') and (oculto='NO') and (cod_concepto<>'VVV')";} 
   if($rango_f=='S'){ $mes_comp='S'; $criterio="rpt_nom_hist WHERE (fecha_p_hasta>='".$cfechad."') and (fecha_p_hasta<='".$cfechah."') and (oculto='NO')  and (cod_concepto<>'VVV') ";   
   ...
   if($rango_f=='S'){ $act_hist='S'; $status=$status." and (fecha_p_hasta>='".$cfechad."') and (fecha_p_hasta<='".$cfechah."')  ";   } 
   else{ if($act_hist=='S'){$status=$status." and (fecha_p_hasta='".$cfechan."') ";}  }   
...
$conn=pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname."");if (pg_ErrorMessage($conn)){ $error=1;}
else{ $Nom_Emp=busca_conf(); if($utf_rpt=="SI"){if($php_os=="WINNT"){$php_os="LINUX";}else{$php_os="WINNT";}}
         if($tipo_nomina_d<>$tipo_nomina_h){$sql="SELECT tipo_nomina,descripcion,desc_grupo from nom001 where tipo_nomina='$tipo_nomina_d'"; $res=pg_query($sql); if($registro=pg_fetch_array($res,0)){$criterio2=$registro["desc_grupo"];}
         }else{$sql="SELECT tipo_nomina,descripcion,desc_grupo from nom001 where tipo_nomina='$tipo_nomina_d'"; $res=pg_query($sql); if($registro=pg_fetch_array($res,0)){$criterio2="Nomina: ".$tipo_nomina_d." ".$registro["descripcion"];}}
    ...
    if($detalle_concepto=="SI"){$nomb_rpt="Rpt_rela_con_cod_pre_rn_re_deta_concep.xml";	$num_rpt=2;}		
	if($detalle_trabajador=="SI"){$nomb_rpt="Rpt_rela_con_cod_pre_rn_re_deta_trab.xml";	$num_rpt=3; }		
	if($tipo_rpt=="HTML"){	
	 // echo $temp." ".$sSQL;	
	...
	if(($tipo_rpt=="PDF")and($num_rpt==1)){ $res=pg_query($sSQL); $filas=pg_num_rows($res); $cod_presup_grupo=""; 
      if($filas>=1){ $registro=pg_fetch_array($res,0); $fechad=$registro["fechad"]; $fechaph=$registro["fechaph"]; }	
      require('../../class/fpdf/fpdf.php');
      class PDF extends FPDF{ 	
		function Header(){ global $criterio2; global $criterio3; global $cod_presup_grupo; global $fechad; global $fechaph;  global $registro; global $rango_f;
	  ...
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $cod_presup=$registro["cod_presup"]; $cod_contable=$registro["cod_contable"];  $denominacion_p=$registro["denominacion_p"]; $linea=$registro["linea"];
		   ...
		   $cod_presup_grupo=$cod_presup.$cod_contable;  $denominacion_p_grupo=$denominacion_p;  
		   ...
		   if($php_os=="WINNT"){$denominacion_p=$registro["denominacion_p"]; }   else{$denominacion_p=utf8_decode($denominacion_p); $denominacion_p=utf8_decode($denominacion_p);}
		} 
		if(($sub_total_monto_asignacion>0)or($sub_total_monto_deduccion>0)or($sub_total_monto>0)){ $sub_total_monto_asignacion=formato_monto($sub_total_monto_asignacion);	$sub_total_monto_deduccion=formato_monto($sub_total_monto_deduccion); $sub_total_monto=formato_monto($sub_total_monto);				    
			$pdf->Cell(43,4,$prev_cod_presup."  ".$prev_cod_contable,0,0,'L');
		    $x=$pdf->GetX();   $y=$pdf->GetY();  $w=100;		   
 ...
 $y=$pdf->GetY();  $t=10;
        if($y>230){$t=30; $pdf->Cell(5,4,'',0,1);  } 
        $pdf->ln($t); $y=$pdf->GetY();
	    if($y<235){$t=235-$y; $pdf->ln($t);} 
	$pdf->SetFont('Arial','',7);
        ...
        $pdf->Cell(50,3,'Lcda. María Pereza',0,0,'C'); //nmdb (20171121) Arendriz Chirinos
		$pdf->Cell(50,3,'T.S.U Arendriz Chirinos',0,0,'C'); //nmdb (20171121) Zuleima Rivas
		$pdf->Cell(50,3,'Lcdo. Jose Medina',0,0,'C');
  
		$pdf->Output();        		
    }
	
	if(($tipo_rpt=="PDF")and($num_rpt==2)){ $res=pg_query($sSQL); $filas=pg_num_rows($res); $cod_presup_grupo=""; 
      if($filas>=1){ $registro=pg_fetch_array($res,0); $fechad=$registro["fechad"]; $fechaph=$registro["fechaph"]; }	
      require('../../class/fpdf/fpdf.php');
      class PDF extends FPDF{ 	
		function Header(){ global $criterio2; global $criterio3; global $cod_presup_grupo; global $fechad; global $fechaph;  global $registro; global $rango_f;
	  ...
	  $pdf->AliasNbPages();
   	  $pdf->AddPage();
	  $pdf->SetFont('Arial','',6);
	  $i=0;  $total_monto_asignacion=0; $total_monto_deduccion=0; $total_monto=0; $sub_total_monto_asignacion=0; $sub_total_monto_deduccion=0; $sub_total_monto=0; $sub_total_monto_asignacion1=0; $sub_total_monto_deduccion1=0; $sub_total_monto1=0; $prev_cod_presup=""; $prev_denominacion_p=""; $prev_cod_concepto=""; $prev_denominacion="";
	  $res=pg_query($sSQL);
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $cod_presup=$registro["cod_presup"]; $denominacion_p=$registro["denominacion_p"];  $cod_concepto=$registro["cod_concepto"]; $denominacion=$registro["denominacion"]; 
		if($php_os=="WINNT"){$denominacion_p=$registro["denominacion_p"];}else{$denominacion_p=utf8_decode($registro["denominacion_p"]);$denominacion=utf8_decode($registro["denominacion"]); }
		   $cod_presup_grupo=$cod_presup; $denominacion_p_grupo=$denominacion_p; $cod_concepto_grupo=$cod_concepto;  $denominacion_grupo=$denominacion;  
		   if($prev_cod_concepto<>$cod_concepto_grupo){ 
			 if(($sub_total_monto_asignacion>0)or($sub_total_monto_deduccion>0)){ $sub_total_monto_asignacion=formato_monto($sub_total_monto_asignacion);	$sub_total_monto_deduccion=formato_monto($sub_total_monto_deduccion); 			    
				$pdf->Cell(10,3,'',0,0,'R');
			 ...
			 $prev_cod_concepto=$cod_concepto_grupo; $prev_denominacion=$denominacion_grupo; $sub_total_monto_asignacion=0; $sub_total_monto_deduccion=0; 
		   }
		   if($prev_cod_presup<>$cod_presup_grupo){ 
			 if(($sub_total_monto_asignacion1>0)or($sub_total_monto_deduccion1>0)or($sub_total_monto1>0)){ $sub_total_monto_asignacion1=formato_monto($sub_total_monto_asignacion1);	$sub_total_monto_deduccion1=formato_monto($sub_total_monto_deduccion1); $sub_total_monto1=formato_monto($sub_total_monto1);	
				...
				$pdf->Ln(3); $sub_total_monto_asignacion1=0; $sub_total_monto_deduccion1=0; $sub_total_monto1=0;
			 }		
			 $pdf->SetFont('Arial','',6);			 
			 $pdf->Cell(32,5,$cod_presup_grupo,0,0,'L');  
			 $pdf->Cell(168,5,$denominacion_p_grupo,0,1,'L');   
			 $prev_cod_presup=$cod_presup_grupo; $prev_denominacion_p=$denominacion_p_grupo; $sub_total_monto_asignacion=0; $sub_total_monto_deduccion=0; $sub_total_monto=0;} 

		   $cod_presup=$registro["cod_presup"]; $denominacion_p=$registro["denominacion_p"]; $cod_concepto=$registro["cod_concepto"]; $denominacion=$registro["denominacion"];      $monto_asignacion=$registro["monto_asignacion"]; $monto_deduccion=$registro["monto_deduccion"]; 
		   $total_monto_asignacion=$total_monto_asignacion+$monto_asignacion;  $total_monto_deduccion=$total_monto_deduccion+$monto_deduccion; 
		   ...
		   $sub_total_monto_deduccion1=$sub_total_monto_deduccion1+$monto_deduccion; $sub_total_monto1=$sub_total_monto1+$monto_asignacion-$monto_deduccion;
		   $monto_asignacion=formato_monto($monto_asignacion); $monto_deduccion=formato_monto($monto_deduccion); 
		   if($php_os=="WINNT"){$denominacion_p=$registro["denominacion_p"]; }   else{$denominacion_p=utf8_decode($denominacion_p); $denominacion=utf8_decode($denominacion);}
		} 
	...
	if(($tipo_rpt=="PDF")and($num_rpt==3)){ $res=pg_query($sSQL); $filas=pg_num_rows($res); $cod_presup_grupo=""; 
      if($filas>=1){ $registro=pg_fetch_array($res,0); $fechad=$registro["fechad"]; $fechaph=$registro["fechaph"]; }	
      require('../../class/fpdf/fpdf.php');
	  ...
	  while($registro=pg_fetch_array($res)){ $i=$i+1; $cod_presup=$registro["cod_presup"]; $denominacion_p=$registro["denominacion_p"];  $cod_concepto=$registro["cod_concepto"]; $denominacion=$registro["denominacion"]; 
		if($php_os=="WINNT"){$denominacion_p=$registro["denominacion_p"];}else{$denominacion_p=utf8_decode($registro["denominacion_p"]);$denominacion=utf8_decode($registro["denominacion"]); }
		   ...
		   if($prev_cod_presup<>$cod_presup_grupo){ 
			 if(($sub_total_monto_asignacion1>0)or($sub_total_monto_deduccion1>0)or($sub_total_monto1>0)){ $sub_total_monto_asignacion1=formato_monto($sub_total_monto_asignacion1);	$sub_total_monto_deduccion1=formato_monto($sub_total_monto_deduccion1); $sub_total_monto1=formato_monto($sub_total_monto1);	
				...
				$pdf->Ln(3); $sub_total_monto_asignacion1=0; $sub_total_monto_deduccion1=0; $sub_total_monto1=0;
				 ...
				 $pdf->Cell(32,5,$cod_presup_grupo,0,0,'L');  
				 $pdf->Cell(168,5,$denominacion_p_grupo,0,1,'L');   
				 $prev_cod_presup=$cod_presup_grupo; $prev_denominacion_p=$denominacion_p_grupo; $sub_total_monto_asignacion=0; $sub_total_monto_deduccion=0; $sub_total_monto=0; $prev_cod_concepto="";} 
            if($prev_cod_concepto<>$cod_concepto_grupo){ 
			    $pdf->Ln(2);
			    $pdf->Cell(10,3,'',0,0,'R');
			 ...
			 $prev_cod_concepto=$cod_concepto_grupo; $prev_denominacion=$denominacion_grupo; 
		   }
		   $cod_presup=$registro["cod_presup"]; $denominacion_p=$registro["denominacion_p"]; $cod_concepto=$registro["cod_concepto"]; $denominacion=$registro["denominacion"];      
		   $nombre=$registro["nombre"]; $cod_empleado=$registro["cod_empleado"];$monto_asignacion=$registro["monto_asignacion"]; $monto_deduccion=$registro["monto_deduccion"]; 
		   $total_monto_asignacion=$total_monto_asignacion+$monto_asignacion;  $total_monto_deduccion=$total_monto_deduccion+$monto_deduccion; 
		   $total_monto=$total_monto+$monto_asignacion-$monto_deduccion;  
		   $sub_total_monto_asignacion=$sub_total_monto_asignacion+$monto_asignacion; 
		   $sub_total_monto_deduccion=$sub_total_monto_deduccion+$monto_deduccion; $sub_total_monto=$sub_total_monto+$monto_asignacion-$monto_deduccion;
		   ...
		   $sub_total_monto_deduccion1=$sub_total_monto_deduccion1+$monto_deduccion; $sub_total_monto1=$sub_total_monto1+$monto_asignacion-$monto_deduccion;
		   $monto_asignacion=formato_monto($monto_asignacion); $monto_deduccion=formato_monto($monto_deduccion); 
		   if($php_os=="WINNT"){$denominacion_p=$registro["denominacion_p"]; }   else{$nombre=utf8_decode($nombre); $denominacion_p=utf8_decode($denominacion_p); $denominacion=utf8_decode($denominacion);}
		    $pdf->SetFont('Arial','',7);
