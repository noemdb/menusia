<?include ("../class/conect.php");  include ("../class/fun_fechas.php");  error_reporting(E_ALL ^ E_NOTICE);
$conn=pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname.""); if (pg_ErrorMessage($conn)) { echo "<p><b>Ocurrio un error conectando a la base de datos: .</b></p>"; exit; }
?>
<script language="JavaScript">
function cerrar_catalogo(cod_empleado,nomb,fecha){
  window.opener.document.forms[0].txtcod_empleado.value = cod_empleado;
  window.opener.document.forms[0].txtnombre.value = nomb;
  window.opener.document.forms[0].txtfecha_ingreso.value = fecha;
...
<?       
		...
		$criterio=""; $criterion="";$criterioc="";
        if ($_GET["criterio"]!=""){ $txt_criterio = $_GET["criterio"]; $txt_criterio = strtoupper ($txt_criterio);
        $criterio = " where (cod_empleado like '%" . $txt_criterio . "%' or nombre like '%" . $txt_criterio . "%') ".$criterioc;}
        ...
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron resultados</font>";
              ...
              if ($_GET["orden"]==""){$orden="cod_empleado";}else{$orden=$_GET["orden"];} $tamPag=10;if ($_GET["pagina"]==""){$pagina=1;$inicio=1;$final=$tamPag;}else{$pagina=$_GET["pagina"];}$limitInf=($pagina-1)*$tamPag;$numPags=ceil($numeroRegistros/$tamPag);
                ...
                echo "<th bgcolor='#00CCFF'><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=fecha_ingreso,cod_empleado&criterio=".$txt_criterio."'>F.Ingreso</a></th>";
                $linea=0;   $Salir=false;
                while($registro=pg_fetch_array($res)){$linea=$linea+1;
                if($linea>$limitInf+$tamPag){$Salir=true;}  if(($linea>=$limitInf) and ($linea<=$limitInf+$tamPag)){$fecha=$registro["fecha_ingreso"]; $fecha=formato_ddmmaaaa($fecha);
?>
<!-- tabla de resultados -->
  <tr bgcolor='#FFFFFF' bordercolor='#000000' onMouseOver="this.style.backgroundColor='#CCCCCC';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#FFFFFF'"o"];" onDblClick="javascript:cerrar_catalogo('<? echo $registro["cod_empleado"]; ?>','<? echo $registro["nombre"]; ?>','<? echo $fecha ?>')" >
...
<form action="Cat_trabajadores_cons.php" method="get">
