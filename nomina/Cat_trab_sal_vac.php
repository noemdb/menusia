...
function cerrar_catalogo(cod_empleado,nomb){
  window.opener.document.forms[0].txtcod_empleado.value = cod_empleado;
  window.opener.document.forms[0].txtnombre.value = nomb;
  window.opener.document.forms[0].txtcod_empleado.focus();
  window.close();
}
</script>
...
<html>
<head>
<title>SIPAP N&Oacute;MINA Y PERSONAL (Catalogo Trabajadores Calculo de Vacaciones)</title>
...
<?
        ...
        $sql="SELECT cod_empleado,cedula,nombre,fecha_c_hasta,fecha_reincorp from TRABAJADOR_VACACION ".$criterio;        $res=pg_query($sql); $numeroRegistros=pg_num_rows($res);
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron resultados</font>";
                ...
                $sql="SELECT cod_empleado,cedula,nombre,fecha_c_hasta,fecha_reincorp from TRABAJADOR_VACACION ".$criterio." ORDER BY ".$orden;  $res=pg_query($sql);
                ...
                echo "<th bgcolor='#00CCFF'><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=nombre&criterio=".$txt_criterio."'>Nombre</a></th>";
				 echo "<th bgcolor='#00CCFF'><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=fecha_reincorp&criterio=".$txt_criterio."'>A Reincorporase</a></th>";
                $linea=0;   $Salir=false;
                while($registro=pg_fetch_array($res)){$linea=$linea+1; $fecha_reincorp=$registro["fecha_reincorp"]; $fecha_reincorp=formato_ddmmaaaa($fecha_reincorp); 
...
<!-- tabla de resultados -->
  <tr bgcolor='#FFFFFF' bordercolor='#000000' onMouseOver="this.style.backgroundColor='#CCCCCC';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#FFFFFF'"o"];" onDblClick="javascript:cerrar_catalogo('<? echo $registro["cod_empleado"]; ?>','<? echo $registro["nombre"]; ?>')" >
   ...
   <td><font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#000033"><b><? echo  $fecha_reincorp; ?></b></font></td>
   </tr>
<?}} echo "</table>";  }?>
        <br>
...
<form action="Cat_trab_sal_vac.php" method="get">
