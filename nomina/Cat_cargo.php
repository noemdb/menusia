...
function cerrar_catalogo(codigo_cargo,des,grado,paso,sueldo){
  window.opener.document.forms[0].txtcodigo_cargo.value = codigo_cargo;
  window.opener.document.forms[0].txtdenominacion.value = des;
  window.opener.document.forms[0].txtpaso.value = paso;
  window.opener.document.forms[0].txtgrado.value = grado;
  window.opener.document.forms[0].txtsueldo.value = sueldo; 
  window.opener.document.forms[0].txtcodigo_cargo.focus();
  window.close();
}
</script>
...
<html>
<head>
<title>SIPAP N&Oacute;MINA Y PERSONAL (Catalogo de Cargos)</title>
...
<?
        ...
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron resultados</font>"; $pagina=1;$inicio=1;$final=1; $numPags=1;
              ...
              if ($_GET["orden"]==""){$orden="codigo_cargo";} else{$orden=$_GET["orden"];} $tamPag=15;if ($_GET["pagina"]==""){$pagina=1;$inicio=1;$final=$tamPag;}else{$pagina=$_GET["pagina"];}$limitInf=($pagina-1)*$tamPag;$numPags=ceil($numeroRegistros/$tamPag);
                ...
                $linea=0;   $Salir=false;
                while($registro=pg_fetch_array($res)) {$linea=$linea+1;  $monto=$registro["sueldo_cargo"]; $monto=formato_monto($monto);
                if  ($linea>$limitInf+$tamPag){$Salir=true;}  if  (($linea>=$limitInf) and ($linea<=$limitInf+$tamPag)){?>
  <tr bgcolor='#FFFFFF' bordercolor='#000000' onMouseOver="this.style.backgroundColor='#CCCCCC';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#FFFFFF'"o"];" onDblClick="javascript:cerrar_catalogo('<? echo $registro["codigo_cargo"]; ?>','<? echo $registro["denominacion"]; ?>','<? echo $registro["grado"]; ?>','<? echo $registro["paso"]; ?>','<? echo $monto; ?>')" >
...
<form action="Cat_cargo.php" method="get">Criterio de b&uacute;squeda:<input type="text" name="criterio" size="22" maxlength="150">
