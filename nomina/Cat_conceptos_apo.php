...
function cerrar_catalogo(cod_concepto,den_concepto){
 window.opener.document.forms[0].txtcod_concepto_a.value = cod_concepto;
 window.opener.document.forms[0].txtdenominacion_a.value = den_concepto;
 window.close();
}
</script>
...
<?      $criterio=""; $txt_criterio=""; $pagina=1;$inicio=1;$final=1; $numPags=1;
        $criterio = "where asig_ded_apo='P'"; $txt_criterio = "";
        if ($_GET){if ($_GET["criterio"]!=""){ $txt_criterio = $_GET["criterio"]; $txt_criterio = strtoupper ($txt_criterio);
        $criterio = " where  asig_ded_apo='P' and cod_concepto like '%" . $txt_criterio . "%' or tipo_nomina like '%" . $txt_criterio . "%' or denominacion like '%" . $txt_criterio . "%' or asignacion like '%" . $txt_criterio . "%' or activo like '%" . $txt_criterio . "%' or oculto like '%" . $txt_criterio . "%'";
        ...
        }else{if ($_GET["orden"]==""){$orden="tipo_nomina,cod_concepto";}else{$orden=$_GET["orden"];} $tamPag=10;if ($_GET["pagina"]==""){$pagina=1;$inicio=1;$final=$tamPag;}else{$pagina=$_GET["pagina"];}$limitInf=($pagina-1)*$tamPag;$numPags=ceil($numeroRegistros/$tamPag);
...
<form action="Cat_conceptos_apo.php" method="get">
