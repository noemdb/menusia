...
function cerrar_catalogo(cod_concepto,den_concepto,conc_aporte,den_aporte){
 window.opener.document.forms[0].txtcod_concepto_r.value = cod_concepto;
 window.opener.document.forms[0].txtdenominacion_r.value = den_concepto;
 window.opener.document.forms[0].txtcod_concepto_a.value = conc_aporte;
 window.opener.document.forms[0].txtdenominacion_a.value = den_aporte;
 window.close();
}
</script>
...
<?      $criterio=""; $txt_criterio=""; $pagina=1;$inicio=1;$final=1; $numPags=1;
        ...
        }} $sql="select a.tipo_nomina,a.cod_concepto,a.denominacion,a.asignacion,a.cod_aporte,b.denominacion as den_aporte from nom002 a left join nom002 b on (b.cod_concepto=a.cod_aporte and a.tipo_nomina=b.tipo_nomina) ".$criterio;  $res=pg_query($sql); $numeroRegistros=pg_num_rows($res); 
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron resultados</font>"; $pagina=1;$inicio=1;$final=1; $numPags=1;
              ...
              if ($_GET["orden"]==""){$orden="tipo_nomina,cod_concepto";}else{$orden=$_GET["orden"];} $tamPag=10;if ($_GET["pagina"]==""){$pagina=1;$inicio=1;$final=$tamPag;}else{$pagina=$_GET["pagina"];}$limitInf=($pagina-1)*$tamPag;$numPags=ceil($numeroRegistros/$tamPag);
                ...
                $sql="select a.tipo_nomina,a.cod_concepto,a.denominacion,a.asignacion,a.cod_aporte,b.denominacion as den_aporte from nom002 a left join nom002 b on (b.cod_concepto=a.cod_aporte and a.tipo_nomina=b.tipo_nomina) ".$criterio." ORDER BY ".$orden;    $res=pg_query($sql);
...
<form action="Cat_conceptos_ret.php" method="get">
