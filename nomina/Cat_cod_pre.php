...
$fformato_presup="XX-XX-XX-XXX-XX-XX-XX";  $formato_categoria="XX-XX-XX";  $formato_partida="XXX-XX-XX-XX";$sql="Select * from SIA005 where campo501='05'";  $resultado=pg_query($sql);
...
<?      $criterio=""; $txt_criterio=""; $pagina=1;$inicio=1;$final=1; $numPags=1; 
        ...
        $sql="select cod_presup,cod_fuente,denominacion from codigos ".$criterio;
        ...
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron Cuentas</font>"; $pagina=1;$inicio=1;$final=1; $numPags=1;
              ...
              if ($_GET["orden"]==""){$orden="cod_presup,cod_fuente";}else{$orden=$_GET["orden"];} $tamPag=10;if ($_GET["pagina"]==""){$pagina=1;$inicio=1;$final=$tamPag;}else{$pagina=$_GET["pagina"];}$limitInf=($pagina-1)*$tamPag;$numPags=ceil($numeroRegistros/$tamPag);
                ...
                $sql="select cod_presup,cod_fuente,denominacion from codigos ".$criterio." ORDER BY ".$orden;  $res=pg_query($sql);
...
<form action="Cat_cod_pre.php" method="get">
