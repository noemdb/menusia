...
<?
        ...
        $sql="SELECT cod_empleado,cedula,nombre,fecha_caus_hasta,fecha_desde from CALCULO_VACACIONES ".$criterio;        $res=pg_query($sql); $numeroRegistros=pg_num_rows($res);
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron resultados</font>";
              ...
              if ($_GET["orden"]==""){$orden="cod_empleado";} else{$orden=$_GET["orden"];} $tamPag=10;if ($_GET["pagina"]==""){$pagina=1;$inicio=1;$final=$tamPag;}else{$pagina=$_GET["pagina"];}$limitInf=($pagina-1)*$tamPag;$numPags=ceil($numeroRegistros/$tamPag);
                ...
                $sql="SELECT cod_empleado,cedula,nombre,fecha_caus_hasta,fecha_desde from CALCULO_VACACIONES ".$criterio." ORDER BY ".$orden;  $res=pg_query($sql);
				...
				echo "<th bgcolor='#00CCFF'><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=fecha_desde&criterio=".$txt_criterio."'>Disfrute Desde</a></th>";
                $linea=0;   $Salir=false;
                while($registro=pg_fetch_array($res)){$linea=$linea+1; $fecha_d_desde=$registro["fecha_desde"]; $fecha_d_desde=formato_ddmmaaaa($fecha_d_desde); 
...
<!-- tabla de resultados -->
  <tr bgcolor='#FFFFFF' bordercolor='#000000' onMouseOver="this.style.backgroundColor='#CCCCCC';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#FFFFFF'"o"];" onDblClick="javascript:cerrar_catalogo('<? echo $registro["cod_empleado"]; ?>','<? echo $registro["nombre"]; ?>')" >
   ...
   <td><font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#000033"><b><? echo $fecha_d_desde; ?></b></font></td>
   </tr>
<?}} echo "</table>";  }?>
        <br>
...
<form action="Cat_trab_cal_vac.php" method="get">
