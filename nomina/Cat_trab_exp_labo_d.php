...
function cerrar_catalogo(cod_empleado){
  window.opener.document.forms[0].txtcod_empleado_d.value = cod_empleado;
  window.opener.document.forms[0].txtcod_empleado_d.focus();
  window.close();
}
</script>
...
<?
        ...
        $sql="SELECT DISTINCT NOM053.Cedula, NOM053.Nombre_E FROM NOM053, NOM055 WHERE NOM053.Cedula = NOM055.Cedula".$criterio;
        ...
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron resultados</font>";
              ...
              if ($_GET["orden"]==""){$orden="cod_empleado";}else{$orden=$_GET["orden"];} $tamPag=10;if ($_GET["pagina"]==""){$pagina=1;$inicio=1;$final=$tamPag;}else{$pagina=$_GET["pagina"];}$limitInf=($pagina-1)*$tamPag;$numPags=ceil($numeroRegistros/$tamPag);
                ...
                $sql="SELECT DISTINCT NOM053.Cedula, NOM053.Nombre_E FROM NOM053, NOM055 WHERE NOM053.Cedula = NOM055.Cedula".$criterio." ORDER BY ".$orden;
                $res=pg_query($sql);
                ...
                echo "<th bgcolor='#00CCFF'><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=nombre_e&criterio=".$txt_criterio."'>Nombre</a></th>";
                $linea=0;   $Salir=false;
...
<form action="../Nomina/Cat_trab_exp_labo_d.php" method="get">
