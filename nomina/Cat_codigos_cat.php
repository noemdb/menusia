...
function cerrar_catalogo(mcodigo,denominacion){
  window.opener.document.forms[0].txtcod_cat_alter.value = mcodigo;
  window.opener.document.forms[0].txtcod_cat_alter.focus();
  window.close();
}
</script>
...
<?
        ...
        if ($_GET["criterio"]!=""){$txt_criterio = $_GET["criterio"]; $txt_criterio = strtoupper ($txt_criterio); $criterio = " where cod_presup_cat like '%" . $txt_criterio . "%' or denominacion_cat like '%" . $txt_criterio . "%'";  }
        ...
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron Cuentas</font>";
              ...
              if ($_GET["orden"]==""){$orden="cod_presup_cat";} else{$orden=$_GET["orden"];} $tamPag=10;if ($_GET["pagina"]==""){$pagina=1;$inicio=1;$final=$tamPag;}else{$pagina=$_GET["pagina"];}$limitInf=($pagina-1)*$tamPag;$numPags=ceil($numeroRegistros/$tamPag);
                ...
                $sql="SELECT cod_presup_cat, cod_fuente_cat, denominacion_cat  FROM pre019 ".$criterio." ORDER BY ".$orden;
                $res=pg_query($sql);
                echo "<table align='center' width='95%' border='1' cellspacing='0' cellpadding='0' bordercolor='#000033' >";
                echo "<th height='25' bgcolor='#99CCFF' ><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=cod_presup_cat&criterio=".$txt_criterio."'>C&oacute;digo Categoria</a></th>";
