...
<?
        ...
        $sql="SELECT  DISTINCT NOM006.Cod_Empleado, NOM006.Nombre FROM NOM023, NOM006 WHERE NOM006.Cod_Empleado = NOM023.Cod_Empleado".$criterio;
        ...
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron resultados</font>";
              ...
              if ($_GET["orden"]==""){$orden="cod_empleado";}else{$orden=$_GET["orden"];} $tamPag=10;if ($_GET["pagina"]==""){$pagina=1;$inicio=1;$final=$tamPag;}else{$pagina=$_GET["pagina"];}$limitInf=($pagina-1)*$tamPag;$numPags=ceil($numeroRegistros/$tamPag);
                ...
                $sql="SELECT  DISTINCT NOM006.Cod_Empleado, NOM006.Nombre FROM NOM023, NOM006 WHERE NOM006.Cod_Empleado = NOM023.Cod_Empleado".$criterio." ORDER BY ".$orden;
                $res=pg_query($sql);
