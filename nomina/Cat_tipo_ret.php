...
function cerrar_catalogo(mtipo_retencion,mdescrip,mtasa,mced_rif,mnombre){
  window.opener.document.forms[0].txtcod_retencion.value = mtipo_retencion;
  window.opener.document.forms[0].txtdescripcion_ret.value = mdescrip;
  window.opener.document.forms[0].txtcod_retencion.focus();
  window.close();
}
</script>
...
<?
        ...
        $txt_criterio = strtoupper ($txt_criterio); $criterio = " where tipo_retencion like '%" . $txt_criterio . "%' or descripcion_ret like '%" . $txt_criterio . "%'"; }
        $sql="SELECT * FROM RETENCIONES ".$criterio;    $res=pg_query($sql);$numeroRegistros=pg_num_rows($res);
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron Tipos de Retenci&oacute;n</font>";
        }else{
              if ($_GET["orden"]==""){$orden="tipo_retencion";}else{$orden=$_GET["orden"];} $tamPag=10;if ($_GET["pagina"]==""){$pagina=1;$inicio=1;$final=$tamPag;}else{$pagina=$_GET["pagina"];}$limitInf=($pagina-1)*$tamPag;$numPags=ceil($numeroRegistros/$tamPag);
                ...
                echo "<th height='20' bgcolor='#99CCFF'><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=descripcion_ret&criterio=".$txt_criterio."'>Descripci&oacute;n</a></th>";
...
<form action="Cat_tipo_ret.php" method="get">
