...
function cerrar_catalogo(codigo_cargo){
  window.opener.document.forms[0].txtcodigo_cargo_h.value = codigo_cargo; 
  window.opener.document.forms[0].txtcodigo_cargo_h.focus();
  window.close();
}
</script>
...
<?
        ...
        }}        $sql="SELECT * FROM nom004 ".$criterio;
        ...
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron resultados</font>"; $pagina=1;$inicio=1;$final=1; $numPags=1;
              ...
              if ($_GET["orden"]==""){$orden="codigo_cargo";}else{$orden=$_GET["orden"];} $tamPag=10;if ($_GET["pagina"]==""){$pagina=1;$inicio=1;$final=$tamPag;}else{$pagina=$_GET["pagina"];}$limitInf=($pagina-1)*$tamPag;$numPags=ceil($numeroRegistros/$tamPag);
...
<form action="Cat_cargos_h.php" method="get">
