...
$formato_presup="XX-XX-XX-XXX-XX-XX-XX";$formato_par="XXX-XX-XX-XX";$sql="Select * from SIA005 where campo501='05'"; $resultado=pg_query($sql); if ($registro=pg_fetch_array($resultado,0)){$formato_presup=$registro["campo504"];$formato_categoria=$registro["campo526"];$formato_par=$registro["campo527"];}$long_c=strlen($formato_presup); $c=strlen($formato_categoria)+2; $p=strlen($formato_partida);
...
function cerrar_catalogo(cod_presup,den_presup){
  window.opener.document.forms[0].txtcod_presup_d.value = cod_presup;
  window.opener.document.forms[0].txtdenominacion_d.value = den_presup;
  window.opener.document.forms[0].txtcod_presup_d.focus();
  window.close();
}
...
<html>
<head>
<title>SIPAP N&Oacute;MINA Y PERSONAL (Catalogo C&oacute;digos Presupuestarios)</title>
...
<?
        ...
        $criterio = " where cod_presup like '%" . $txt_criterio . "%' or denominacion like '%" . $txt_criterio . "%'";
        }
        $sql="SELECT * FROM pre001 ".$criterio;
        ...
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron resultados</font>";
              ...
              if ($_GET["orden"]==""){$orden="cod_presup";}else{$orden=$_GET["orden"];} $tamPag=10;if ($_GET["pagina"]==""){$pagina=1;$inicio=1;$final=$tamPag;}else{$pagina=$_GET["pagina"];}$limitInf=($pagina-1)*$tamPag;$numPags=ceil($numeroRegistros/$tamPag);
                ...
                $sql="SELECT * FROM pre001 ".$criterio." ORDER BY ".$orden;
                $res=pg_query($sql);
                echo "<table align='center' width='95%' border='1' cellspacing='0' cellpadding='0' bordercolor='#000033' >";
                echo "<th bgcolor='#00CCFF' ><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=cod_presup&criterio=".$txt_criterio."'>C&oacute;digo</a></th>";
...
?>
<!-- tabla de resultados -->
  <tr bgcolor='#FFFFFF' bordercolor='#000000' onMouseOver="this.style.backgroundColor='#CCCCCC';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#FFFFFF'"o"];" onDblClick="javascript:cerrar_catalogo('<? echo $registro["cod_presup"]; ?>','<? echo $registro["denominacion"]; ?>')" >
...
<form action="Cat_codigos_pred.php" method="get" onSubmit="return revisar()">
<table border="0">
  <tr>
	<td>
