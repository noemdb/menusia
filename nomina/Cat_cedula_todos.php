...
function cerrar_catalogo(cedula,nomb){
  window.opener.document.forms[0].txtcedula.value = cedula;
  window.opener.document.forms[0].txtnombre.value = nomb;
  window.opener.document.forms[0].txtcedula.focus();
  window.close();
}
</script>
...
<html>
<head>
<title>SIPAP N&Oacute;MINA Y PERSONAL (Catalogo Cedula de Trabajadores)</title>
<meta http-equiv="Content-type" content="text/html;charset=UTF-8">
<meta http-equiv="Pragma" content="no-cache" />
<LINK href="../class/sia.css" type="text/css" rel="stylesheet">
</head><body>
<?
        $criterio=""; $txt_criterio=""; $pagina=1;$inicio=1;$final=1; $numPags=1;  $criterion="where (cedula<>'') and (substring(nombre,1,7)<>'VACANTE')   "; $criterioc="";
		$criterio=""; //if ($gnomina=="00"){  }else{ $criterion=" where tipo_nomina='$gnomina' ";  $criterioc=" and tipo_nomina='$gnomina' ";}
        $criterio=$criterion;
        if ($_GET){if ($_GET["criterio"]!=""){ $txt_criterio = $_GET["criterio"]; $txt_criterio = strtoupper ($txt_criterio);
        $criterio = " where (cedula<>'') and (nombre<>'VACANTE') and (cedula like '%" . $txt_criterio . "%' or nombre like '%" . $txt_criterio . "%') ".$criterioc;}}
        $sql="SELECT cedula,nombre FROM nom006 ".$criterio;        $res=pg_query($sql); $numeroRegistros=pg_num_rows($res);
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron resultados</font>";
              ...
              if ($_GET["orden"]==""){$orden="cedula";}else{$orden=$_GET["orden"];} $tamPag=10;if ($_GET["pagina"]==""){$pagina=1;$inicio=1;$final=$tamPag;}else{$pagina=$_GET["pagina"];}$limitInf=($pagina-1)*$tamPag;$numPags=ceil($numeroRegistros/$tamPag);
                ...
                echo "<th bgcolor='#00CCFF' ><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=cedula&criterio=".$txt_criterio."'>Cedula Trabajador</a></th>";
...
<form action="Cat_cedula_todos.php" method="get">
