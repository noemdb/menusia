...
function cerrar_catalogo(cod_concepto,den_concepto){
 ...
 window.opener.document.forms[0].txtcod_concepto.focus();
 window.close();
}
</script>
...
<?
        ...
        $criterio = " where  (cod_concepto like '%" . $txt_criterio . "%' or tipo_nomina like '%" . $txt_criterio . "%' or denominacion like '%" . $txt_criterio . "%' or asignacion like '%" . $txt_criterio . "%' or activo like '%" . $txt_criterio . "%' or oculto like '%" .$txt_criterio."%') ".$criterioc;}  }
        $sql="SELECT * FROM nom002 ".$criterio; $res=pg_query($sql); $numeroRegistros=pg_num_rows($res);
		 if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron resultados</font>";
              ...
              if ($_GET["orden"]==""){$orden="tipo_nomina,cod_concepto";}else{$orden=$_GET["orden"];} $tamPag=10;if ($_GET["pagina"]==""){$pagina=1;$inicio=1;$final=$tamPag;}else{$pagina=$_GET["pagina"];}$limitInf=($pagina-1)*$tamPag;$numPags=ceil($numeroRegistros/$tamPag);
                ...
                echo "<th bgcolor='#00CCFF' ><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=tipo_nomina,cod_concepto&criterio=".$txt_criterio."'>Tipo Nomina</a></th>";
                echo "<th bgcolor='#00CCFF' ><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=cod_concepto,tipo_nomina&criterio=".$txt_criterio."'>Codigo</a></th>";
                echo "<th bgcolor='#00CCFF' ><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=denominacion,tipo_nomina,cod_concepto&criterio=".$txt_criterio."'>Denominacion del Concepto</a></th>";
                echo "<th bgcolor='#00CCFF' ><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=asignacion,tipo_nomina,cod_concepto&criterio=".$txt_criterio."'>Asignacion</a></th>";
                echo "<th bgcolor='#00CCFF' ><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=activo,tipo_nomina,cod_concepto&criterio=".$txt_criterio."'>Activo</a></th>";
                echo "<th bgcolor='#00CCFF' ><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=oculto,tipo_nomina,cod_concepto&criterio=".$txt_criterio."'>Oculto</a></th>";
                $linea=0;   $Salir=false;
...
<form action="Cat_conceptos.php" method="get">
