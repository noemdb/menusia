...
<html>
<head>
<title>SIPAP N&Oacute;MINA Y PERSONAL (Catalogo Calculo de Prestaciones)</title>
<meta http-equiv="Content-type" content="text/html;charset=UTF-8">
<LINK href="../class/sia.css" type="text/css" rel="stylesheet">
<script language="JavaScript" type="text/JavaScript">
function CargarUrl(mcodigo){var murl;  murl="Act_cal_prest_lott.php?Gcodigo="+mcodigo;   document.location = murl;}
</script></head>
<body>
...
<?      $criterio=""; $txt_criterio=""; $pagina=1;$inicio=1;$final=1; $numPags=1;  $criterio = " Where (cod_empleado in (select cod_empleado from nom030 ))";
        if ($_GET){if ($_GET["criterio"]!=""){$txt_criterio = $_GET["criterio"];  $txt_criterio=strtoupper($txt_criterio);
        $criterio = "Where (cod_empleado in (select cod_empleado from nom030 )) and (cod_empleado  like '%" . $txt_criterio . "%' or cedula like '%" . $txt_criterio . "%' or nombre like '%" . $txt_criterio . "%')";}}
        $sql="SELECT cod_empleado,cedula,nombre FROM nom006 ".$criterio;  $res=pg_query($sql);$numeroRegistros=pg_num_rows($res);
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron resultados</font>";
                ...
                $sql="SELECT cod_empleado,cedula,nombre FROM nom006 ".$criterio." ORDER BY ".$orden;  $res=pg_query($sql);
...
<form action="Cat_cal_presta_lott.php" method="get">Criterio de b&uacute;squeda:<input type="text" name="criterio" size="22" maxlength="150"> <input type="submit" value="Buscar">
</div>
</form>
</body>
</html>
