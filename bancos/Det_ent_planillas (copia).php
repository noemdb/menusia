<?php include ("../class/conect.php");  include ("../class/funciones.php");    $fecha_hoy=asigna_fecha_hoy(); $equipo=getenv("COMPUTERNAME");  $mcod_m="BAN013".$usuario_sia.$equipo; $codigo_mov=substr($mcod_m,0,49);
...
<html>
<head>
<title>SIPAP CONTROL BANCARIO (Detalles Planillas de Retencion)</title>
...
function Llama_Modificar(planilla,nro_planilla){var murl; var cod_ret; var fechae; var deposito; var banco;
 ...
 murl="Mod_planilla_ent.php?planilla="+planilla+"&nro_planilla="+nro_planilla+"&pdesde=<?php echo $plan_desde?>&phasta=<?php echo $plan_hasta?>&fdesde=<?php echo $fecha_desde?>&fhasta=<?php echo $fecha_hasta?>"+"&cod_ret="+cod_ret+"&fechae="+fechae+"&deposito="+deposito+"&banco="+banco; document.location=murl; }
function Entera_todos(){ var url; var cod_ret; var fechae; var deposito; var banco; var r;
 cod_ret=document.form1.txtcod_ret.value; fechae=document.form1.txtfecha_ent.value;  deposito=document.form1.txtnro_deposito.value; banco=document.form1.txtnomb_banco.value;
 r=confirm("Registrar todas las planillas como Enterada ?");  if (r==true){url="entera_todas.php?planilla=<?php echo $tipo_planilla?>&pdesde=<?php echo $plan_desde?>&phasta=<?php echo $plan_hasta?>&fdesde=<?php echo $fecha_desde?>&fhasta=<?php echo $fecha_hasta?>"+"&cod_ret="+cod_ret+"&fechae="+fechae+"&deposito="+deposito+"&banco="+banco; document.location=url; }
}
</script>
<body>
   ...
   <tr>
      <td><table width="940">
          <tr>
            ...
            <td width="80"><span class="Estilo5"><input class="Estilo10" name="txtcod_ret" type="text" id="txtcod_ret" size="3" maxlength="2"  onFocus="encender(this)" onBlur="apagar(this)"  onchange="chequea_cod_ret(this.form);" value="<?php echo $cod_ret;?>"  >  </span> </td>
            <td width="115"><span class="Estilo5">FECHA ENTERADO : </span></td>
            <td width="120"><span class="Estilo5"> <input class="Estilo10" name="txtfecha_ent" type="text" id="txtfecha_ent" size="12" maxlength="10"  onFocus="encender(this)" onBlur="apagar(this)"  value="<?php echo $fecha_ent;?>">  </span> </td>
            <td width="115"><span class="Estilo5">NRO. DEPOSITO : </span></td>
            <td width="150"><span class="Estilo5"><input class="Estilo10" name="txtnro_deposito" type="text" id="txtnro_deposito" size="10" maxlength="8"  onFocus="encender(this)" onBlur="apagar(this)" value="<?php echo $nro_deposito;?>"> </span></td>
    ...
    <tr>
      <td><table width="940">
          <tr>
            ...
            <td width="580"><span class="Estilo5"><input class="Estilo10" name="txtnomb_banco" type="text" id="txtnomb_banco" size="100" maxlength="80"  onFocus="encender(this)" onBlur="apagar(this)"  onchange="chequea_cod_ret(this.form);" value="<?php echo $nom_banco;?>" >  </span> </td>
...
while($registro=pg_fetch_array($res)){ $monto_r=formato_monto($registro["monto_retencion"]);
  ...
  if($cod_retencion!="00"){$fecha_ent=formato_ddmmaaaa($fecha_enterado); $total=$total+$registro["monto_retencion"];} else{$total=$total; $fecha_ent="";}
  $sfecha=$registro["fecha_emision"];  $fecha = substr($sfecha,8,2)."/".substr($sfecha,5,2)."/".substr($sfecha,0,4);
?>
         <tr bgcolor='#FFFFFF' bordercolor='#000000' height="20" class="Estilo5" onMouseOver="this.style.backgroundColor='#CCCCCC';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#FFFFFF'"o"];" onDblClick="javascript:Llama_Modificar('<?php  echo $registro["tipo_planilla"]; ?>','<?php  echo $registro["nro_planilla"]; ?>');">
           <td width="80" align="left"><?php  echo $registro["nro_planilla"]; ?></td>
           ...
           <td width="40" align="left"><?php  echo $cod_retencion; ?></td>
           <td width="100" align="left"><?php  echo $fecha_ent; ?></td>
           <td width="250" align="left"><?php  echo $nombre_b; ?></td>
           <td width="80" align="left"><?php  echo $deposito; ?></td>
           ...
           <td width="140" align="right"><?php  echo $monto_p; ?></td>
           <td width="100" align="left"><?php  echo $registro["ced_rif"]; ?></td>
           <td width="330" align="left"><?php  echo $registro["nombre"]; ?></td>
         </tr>
