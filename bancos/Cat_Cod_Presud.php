...
function Llamar_cat(){var murl;
  murl= "Cat_Cod_Presud.php?criterio="+document.form1.criterio.value;  LlamarURL(murl);}
function cerrar_catalogo(mcodigo,mnombre){
  window.opener.document.forms[0].txtcodigo_pre_d.value=mcodigo;
  window.opener.document.forms[0].txtcodigo_pre_d.focus();
  window.close();
}
</script>
...
<html>
<head>
<title>SIPAP ORDENAMIENTO DE PAGO (Catalogo de Codigos Presupuestarios)</title>
...
<?
        ...
        if ($_GET["criterio"]!=""){ $txt_criterio=$_GET["criterio"]; $txt_criterio=strtoupper ($txt_criterio);  $criterio=" where cod_presup like '%" . $txt_criterio . "%' or denominacion like '%" . $txt_criterio . "%'";  }
