...
function Llamar_cat(){var murl;  murl= "Cat_Bancosd.php?criterio="+document.form1.criterio.value;  LlamarURL(murl);}
function cerrar_catalogo(mcodigo,mnombre){
  window.opener.document.forms[0].txtcod_banco_d.value=mcodigo;
  window.opener.document.forms[0].txtdesc_banco_d.value=mnombre;
  window.opener.document.forms[0].txtcod_banco_d.focus();
  window.close();
}
</script>
...
?>
  <tr bgcolor='#FFFFFF' bordercolor='#000000' onMouseOver="this.style.backgroundColor='#CCCCCC';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#FFFFFF'"o"];" onDblClick="javascript:cerrar_catalogo('<?php  echo $registro["cod_banco"]?>','<?php  echo $registro["nombre_banco"]; ?>','<?php  echo $registro["nro_cuenta"]; ?>','<?php  echo $registro["descripcion_banco"]; ?>')" >
    ...
    <td><font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#000033"><b><?php  echo $registro["descripcion_banco"]; ?></b></font></td>
  </tr>
<?php } } echo "</table>"; }
?>
        <br>
