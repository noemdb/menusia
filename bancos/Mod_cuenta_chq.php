...
function llamar_eliminar(codigo_mov,codigo,dc){var murl;var r;
 if (codigo=="") {alert("Codigo debe ser Seleccionado");} else { murl="Esta seguro en Eliminar el Codigo:"+codigo+" D/C:"+dc+" de la Orden ?";
  ...
  if(r==true){r=confirm("Esta Realmente seguro en Eliminar el Codigo de Cuenta de la orden ?");
    if(r==true){murl="Delete_cuenta_chq.php?codigo_mov="+codigo_mov+"&cod_cuenta="+codigo+"&debito_credito="+dc; document.location=murl;} }else { url="Cancelado, no elimino"; }
  }
}
function revisar(){
...
<?
$conn=pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname."");  $nombre_cuenta="";$monto_asiento=0;
...
?>
<body>
<form name="form1" method="post" action="Update_cuenta_chq.php" onSubmit="return revisar()">
  <table width="623" height="70" border="1" align="center" cellpadding="0" cellspacing="0">
    <tr>
      <td><table width="620" border="0" cellpadding="0" cellspacing="0">
        <tr>
          <td height="31" align="center" bgcolor="#000066"><span class="Estilo9">MODIFICAR CUENTA DEL COMPROBANTE DEL CHEQUE </span></td>
        </tr>
        <tr>
          <td>&nbsp;</td>
        </tr>
