...
function Llamar_cat(){var murl;
  murl= "Cat_Tipo_Retencionesd.php?criterio="+document.form1.criterio.value;
  LlamarURL(murl);
}
function cerrar_catalogo(mcodigo,mnombre){
  window.opener.document.forms[0].txttipo_reten_d.value = mcodigo;
  window.opener.document.forms[0].txttipo_reten_d.focus();
  window.close();
}
</script>
...
?>
  <tr bgcolor='#FFFFFF' bordercolor='#000000' onMouseOver="this.style.backgroundColor='#CCCCCC';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#FFFFFF'"o"];" onDblClick="javascript:cerrar_catalogo('<? echo $registro["tipo_retencion"]?>','<? echo $registro["descripcion_ret"]; ?>')" >
