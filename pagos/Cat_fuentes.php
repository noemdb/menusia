...
function cerrar_catalogo(mcuenta,mnombre){
  window.opener.document.forms[0].txtcod_fuente.value = mcuenta;
  window.opener.document.forms[0].txtdes_fuente.value = mnombre;
  window.opener.document.forms[0].txtcod_fuente.focus();
  window.close();
}
</script>
...
<?php 
       $criterio=""; $txt_criterio=""; $pagina=1;$inicio=1;$final=1;
        ...
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron Fuentes</font>";$pagina=1; $inicio=1; $final=1; $numPags=1; 
        }else{
              if ($_GET["orden"]==""){$orden="cod_fuente_financ";}else{$orden=$_GET["orden"];}  $tamPag=10;
                ...
                echo "<th height='20' bgcolor='#99CCFF' ><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=cod_fuente_financ&criterio=".$txt_criterio."'>Partida</a></th>";
  ...
  <tr bgcolor='#FFFFFF' bordercolor='#000000' onMouseOver="this.style.backgroundColor='#CCCCCC';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#FFFFFF'"o"];" onDblClick="javascript:cerrar_catalogo('<?php  echo $registro["cod_fuente_financ"]; ?>','<?php  echo $registro["des_fuente_financ"]; ?>');" >
