...
function cerrar_catalogo(mtipo_comp,mref_comp,mcod_comp,mnomb_abrev,mced_rif,mnombre,mfunc_inv,mdescrip,mtipord,mtasaant,mcodant){
  ...
  window.opener.document.forms[0].txtconcepto.value = mdescrip;
  window.opener.document.forms[0].txttipo_orden.value = mtipord;
  window.opener.document.forms[0].txttasa_ant.value = mtasaant;
  window.opener.document.forms[0].txtCodigo_Cuenta.value = mcodant;
...
<?      $criterio=""; $txt_criterio=""; $pagina=1;$inicio=1;$final=1;
        $criterio = "where (anulado='N') and (tipo_compromiso<>'0000') and (tipo_compromiso<>'A000') and (tiene_anticipo='S' or tasa_anticipo>0)";
        ...
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron Compromisos</font>";$pagina=1; $inicio=1; $final=1; $numPags=1; 
        }else{if ($_GET["orden"]==""){$orden="tipo_compromiso,referencia_comp";}else{$orden=$_GET["orden"];}
                $tamPag=10;
                ...
                echo "<th bgcolor='#99CCFF'><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=referencia_comp&criterio=".$txt_criterio."'>Referencia</a></th>";
                ...
                echo "<th bgcolor='#99CCFF'><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=descripcion_comp&criterio=".$txt_criterio."'>Descripcion</a></th>";
                $linea=0;
                $Salir=false;
...
<form action="Cat_comp_ant.php" method="get">
