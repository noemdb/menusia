...
function Llamar_cat(){var murl;
  murl= "Cat_Tipo_Retencionesh.php?criterio="+document.form1.criterio.value;
  LlamarURL(murl);
}
function cerrar_catalogo(mcodigo,mnombre){
  window.opener.document.forms[0].txttipo_reten_h.value = mcodigo;
  window.opener.document.forms[0].txttipo_reten_h.focus();
  window.close();
}
</script>
...
<?php 
        ...
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron resultados</font>";$pagina=1; $inicio=1; $final=1; $numPags=1; 
        }else{if ($_GET["orden"]==""){$orden="tipo_retencion";} else{$orden=$_GET["orden"];}  $tamPag=10;
                ...
                echo "<th height='20' bgcolor='#99CCFF' ><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=tipo_retencion&criterio=".$txt_criterio."'>Tipo Retencion/Rif</a></th>";
...
?>
  <tr bgcolor='#FFFFFF' bordercolor='#000000' onMouseOver="this.style.backgroundColor='#CCCCCC';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#FFFFFF'"o"];" onDblClick="javascript:cerrar_catalogo('<?php  echo $registro["tipo_retencion"]?>','<?php  echo $registro["descripcion_ret"]; ?>')" >
    <td><font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#000033"><b><?php  echo $registro["tipo_retencion"]; ?></b></font></td>
    <td><font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#000033"><b><?php  echo $registro["descripcion_ret"]; ?></b></font></td>
  </tr>
<?php } } echo "</table>"; }
?>
        <br>
