...
function Llamar_cat(){var murl;
  murl= "Cat_tipo_reth.php?criterio="+document.form1.criterio.value;
  LlamarURL(murl);
}
function cerrar_catalogo(mcodigo,mnombre){
  window.opener.document.forms[0].txttipo_ret_h.value = mcodigo;
  window.opener.document.forms[0].txttipo_ret_h.focus();
  window.close();
}
</script>
...
<?
        ...
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron resultados</font>";$pagina=1; $inicio=1; $final=1; $numPags=1; 
              ...
              if ($_GET["orden"]==""){$orden="tipo_retencion";}else{$orden=$_GET["orden"];}  $tamPag=10;
                ...
                echo "<th height='20' bgcolor='#99CCFF' ><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=tipo_retencion&criterio=".$txt_criterio."'>Tipo Retencion</a></th>";
