...
function Llamar_cat(){
var murl;
  murl= "Cat_patidad.php?criterio="+document.form1.criterio.value;
  LlamarURL(murl);
}
function cerrar_catalogo(mcod_presup,mdenominacion,mcod_contable){
  window.opener.document.forms[0].txtcod_partida.value = mcod_presup;
  window.opener.document.forms[0].txtcod_contable.value = mcod_contable;
...
<?
       $criterio=""; $txt_criterio=""; $pagina=1;$inicio=1;$final=1;
        ...
        $criterio = " where cod_presup like '%" . $txt_criterio . "%' or denominacion like '%" . $txt_criterio . "%' or cod_contable like '%" . $txt_criterio . "%'";        }}
        ...
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron resultados</font>";$pagina=1; $inicio=1; $final=1; $numPags=1; 
              ...
              if ($_GET["orden"]==""){$orden="cod_presup";}else{$orden=$_GET["orden"];}  $tamPag=10;
                ...
                $sql="SELECT * FROM CODIGOS WHERE cod_contable<>''".$criterio." ORDER BY ".$orden;    $res=pg_query($sql);
                echo "<table align='center' width='95%' border='1' cellspacing='0' cellpadding='0' bordercolor='#000033' >";
                echo "<th height='20' bgcolor='#99CCFF' ><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=cod_presup&criterio=".$txt_criterio."'>Codigo Partida</a></th>";
                ...
                echo "<th height='20' bgcolor='#99CCFF'><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=cod_contable&criterio=".$txt_criterio."'>Codigo Contable</a></th>";
                $linea=0;
                $Salir=false;
...
?>
  <tr bgcolor='#FFFFFF' bordercolor='#000000' onMouseOver="this.style.backgroundColor='#CCCCCC';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#FFFFFF'"o"];" onDblClick="javascript:cerrar_catalogo('<? echo $registro["cod_presup"]?>', '<? echo $registro["denominacion"]?>', '<? echo $registro["cod_contable"]; ?>')" >
