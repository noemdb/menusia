...
<html>
<head>
<title>SIPAP ORDENAMIENTO DE PAGOS (Catalogo de Bancos)</title>
...
<?
        ...
        $criterio = " where cod_banco like '%" . $txt_criterio . "%' or nombre_banco like '%" . $txt_criterio . "%'";   }}
        ...
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron Beneficiarios</font>";$pagina=1; $inicio=1; $final=1; $numPags=1; 
              ...
              if ($_GET["orden"]==""){$orden="cod_banco";}else{$orden=$_GET["orden"];}  $tamPag=10;
                ...
                echo "<th height='20' bgcolor='#99CCFF' ><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=cod_banco&criterio=".$txt_criterio."'>Codigo</a></th>";
