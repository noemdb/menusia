<?php include ("../class/conect.php"); error_reporting(E_ALL ^ E_NOTICE); include ("../class/fun_numeros.php"); include ("../class/fun_fechas.php");
$conn=pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname."");
if (pg_last_error($conn)) { echo "<p><b>Ocurrio un error conectando a la base de datos: .</b></p>"; exit; }
?>
<script language="JavaScript">
function cerrar_catalogo(mcodigo,mdenominacion,mmonto,mfecha){
  window.opener.document.forms[0].txtcod_bien_aso.value = mcodigo;
  window.opener.document.forms[0].txtdenominacion.value = mdenominacion;
  window.opener.document.forms[0].txtvalor_rep.value = mmonto;
  window.opener.document.forms[0].txtfecha_rep.value = mfecha;
  window.opener.document.forms[0].txtcod_bien_aso.focus();
  window.close();
}
</script>
...
<?php 
        ...
        $sql="SELECT cod_bien_mue,denominacion,marca,num_bien,modelo,color,matricula,serial1,serial2,valor_incorporacion,valor_incorporacion,fecha_incorporacion  FROM BIEN015 ".$criterio;$res=pg_query($sql);$numeroRegistros=pg_num_rows($res);
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron Bienes</font>";$pagina=1; $inicio=1; $final=1; $numPags=1;
              ...
              if ($_GET["orden"]==""){$orden="cod_bien_mue";} else{$orden=$_GET["orden"];} $tamPag=10;
                ...
                $sql="SELECT cod_bien_mue,num_bien,denominacion,marca,modelo,color,matricula,serial1,serial2,valor_incorporacion,fecha_incorporacion  FROM BIEN015 ".$criterio." ORDER BY ".$orden; $res=pg_query($sql);
                ...
                $linea=0; $Salir=false;
                while($registro=pg_fetch_array($res)){ $linea=$linea+1; $cod_depe=$registro["cod_bien_mue"]; $monto=$registro["valor_incorporacion"]; $monto=formato_monto($monto); $fecha_incorporacion=$registro["fecha_incorporacion"]; $fecha_in=formato_ddmmaaaa($fecha_incorporacion);
...
?>
  <tr bgcolor='#FFFFFF' bordercolor='#000000' onMouseOver="this.style.backgroundColor='#CCCCCC';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#FFFFFF'"o"];" onDblClick="javascript:cerrar_catalogo('<?php  echo $registro["cod_bien_mue"]; ?>','<?php  echo $denominacion; ?>','<?php  echo $monto; ?>','<?php  echo $fecha_in; ?>')" >
    ...
    <td align="right"><font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#000033"><b><?php  echo $monto; ?></b></font></td>
  </tr>
<?php } }echo "</table>"; }
?>
        <br>
...
<form action="Cat_bienes_muebles_mejora.php" method="get">
