...
<?php 
        ...
        $sql="SELECT cod_bien_mue,denominacion,vida_util, valor_residual,cod_presup_dep,monto_depreciado,cod_contablea, cod_contabled  FROM BIEN015 ".$criterio;$res=pg_query($sql);$numeroRegistros=pg_num_rows($res);
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron Dependencias</font>";$pagina=1; $inicio=1; $final=1; $numPags=1;
              ...
              if ($_GET["orden"]==""){$orden="cod_bien_mue";} else{$orden=$_GET["orden"];} $tamPag=10;
                ...
                $sql="SELECT cod_bien_mue,denominacion,vida_util,valor_residual,cod_presup_dep,monto_depreciado,cod_contablea, cod_contabled  FROM BIEN015 ".$criterio." ORDER BY ".$orden; $res=pg_query($sql);
...
<form action="Cat_bienes_muebles_depreciadosd.php" method="get">
