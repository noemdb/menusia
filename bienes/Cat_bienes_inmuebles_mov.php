...
<html>
<head>
<title>SIPAP CONTROL DE BIENES NACIONALES (Catalogo de Bienes Inmuebles)</title>
...
<?php 
        ...
        $criterio = " where cod_bien_inm like '%" . $txt_criterio . "%' or denominacion like '%" . $txt_criterio . "%' or num_bien like '%" .  $txt_criterio . "%'"; }}
        $sql="SELECT cod_bien_inm,denominacion,num_bien,fecha_incorporacion,valor_incorporacion  FROM bien014 ".$criterio;$res=pg_query($sql);$numeroRegistros=pg_num_rows($res);
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron Bienes</font>";$pagina=1; $inicio=1; $final=1; $numPags=1;
              ...
              if ($_GET["orden"]==""){$orden="cod_bien_inm";} else{$orden=$_GET["orden"];} $tamPag=10;
                ...
                $sql="SELECT cod_bien_inm,num_bien,denominacion,fecha_incorporacion,valor_incorporacion  FROM bien014 ".$criterio." ORDER BY ".$orden; $res=pg_query($sql);
                echo "<table align='center' width='95%' border='1' cellspacing='0' cellpadding='0' bordercolor='#000033' >";
                echo "<th height='25' bgcolor='#99CCFF' ><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=cod_bien_inm&criterio=".$txt_criterio."'>Codigo de Bien Mueble</a></th>";
                ...
                $linea=0; $Salir=false;
                while($registro=pg_fetch_array($res)){ $linea=$linea+1; $cod_depe=$registro["cod_bien_inm"]; $monto=$registro["valor_incorporacion"]; $monto=formato_monto($monto); 
                if  ($linea>$limitInf+$tamPag){$Salir=true;}
                if  (($linea>=$limitInf) and ($linea<=$limitInf+$tamPag)){
?>
  <tr bgcolor='#FFFFFF' bordercolor='#000000' onMouseOver="this.style.backgroundColor='#CCCCCC';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#FFFFFF'"o"];" onDblClick="javascript:cerrar_catalogo('<?php  echo $registro["cod_bien_inm"]; ?>','<?php  echo $registro["denominacion"]; ?>','<?php  echo $monto; ?>')" >
    ...
    <td><font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#000033"><b><?php  echo $monto; ?></b></font></td>
  </tr>
<?php } }echo "</table>"; }
?>
        <br>
...
<form action="Cat_bienes_inmuebles_mov.php" method="get">
