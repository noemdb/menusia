...
function cerrar_catalogo(mcodigo,mdenominacion){
  window.opener.document.forms[0].txtcod_departamentod.value = mcodigo;
...
<html>
<head>
<title>SIPAP CONTROL DE BIENES NACIONALES (Catalogo de Departamentos)</title>
...
<?php 
        ...
        $sql="SELECT cod_dependencia,cod_direccion,cod_departamento,denominacion_dep  FROM bien006  where cod_dependencia='$cod_dependen' and cod_direccion='$cod_direccion' ".$criterio;$res=pg_query($sql);$numeroRegistros=pg_num_rows($res);
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron Departamentos</font>";$pagina=1; $inicio=1; $final=1; $numPags=1;
              ...
              if ($_GET["orden"]==""){$orden="cod_dependencia,cod_direccion,cod_departamento";} else{$orden=$_GET["orden"];} $tamPag=10;
                ...
                $sql="SELECT cod_dependencia,cod_direccion,cod_departamento,denominacion_dep  FROM bien006 where cod_dependencia='$cod_dependen' and cod_direccion='$cod_direccion' ".$criterio." ORDER BY ".$orden; $res=pg_query($sql);
...
<form action="Cat_departamentosd.php" method="get">
