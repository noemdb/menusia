...
<html>
<head>
<title>SIPAP CONTROL DE BIENES NACIONALES (Catalogo de Clasificacion)</title>
...
<?php       $criterio=""; $txt_criterio=""; $pagina=1;$inicio=1;$final=1;
        ...
        $sql="SELECT grupo_c,codigo_c,denominacion_c  FROM BIEN008 ".$criterio;$res=pg_query($sql);$numeroRegistros=pg_num_rows($res);
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron Dependencias</font>";$pagina=1; $inicio=1; $final=1; $numPags=1;
              ...
              if ($_GET["orden"]==""){$orden="grupo_c,codigo_c";} else{$orden=$_GET["orden"];} $tamPag=10;
                ...
                $sql="SELECT grupo_c,codigo_c,denominacion_c  FROM BIEN008".$criterio." ORDER BY ".$orden; $res=pg_query($sql);
                ...
                $linea=0; $Salir=false;
                while($registro=pg_fetch_array($res)){ $linea=$linea+1; $cod_depe=$registro["codigo_c"];
                if  ($linea>$limitInf+$tamPag){$Salir=true;}
                if  (($linea>=$limitInf) and ($linea<=$limitInf+$tamPag)){
?>
  <tr bgcolor='#FFFFFF' bordercolor='#000000' onMouseOver="this.style.backgroundColor='#CCCCCC';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#FFFFFF'"o"];" onDblClick="javascript:cerrar_catalogo('<?php  echo $registro["codigo_c"]; ?>','<?php  echo $registro["denominacion_c"]; ?>')" >
...
<form action="Cat_clasificaciond.php" method="get">
