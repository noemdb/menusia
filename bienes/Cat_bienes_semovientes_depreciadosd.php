...
<?php 
        ...
        $sql="SELECT cod_bien_sem,denominacion,vida_util, valor_residual,cod_presup_dep,monto_depreciado,cod_contablea, cod_contabled  FROM BIEN016 ".$criterio;$res=pg_query($sql);$numeroRegistros=pg_num_rows($res);
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron Dependencias</font>";$pagina=1; $inicio=1; $final=1; $numPags=1;
              ...
              if ($_GET["orden"]==""){$orden="cod_bien_sem";} else{$orden=$_GET["orden"];} $tamPag=10;
                ...
                $sql="SELECT cod_bien_sem,denominacion,vida_util,valor_residual,cod_presup_dep,monto_depreciado,cod_contablea, cod_contabled  FROM BIEN016 ".$criterio." ORDER BY ".$orden; $res=pg_query($sql);
                echo "<table align='center' width='95%' border='1' cellspacing='0' cellpadding='0' bordercolor='#000033' >";
                echo "<th height='25' bgcolor='#99CCFF' ><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=cod_bien_sem&criterio=".$txt_criterio."'>Codigo de Bien Semovientes</a></th>";
...
<form action="Cat_bienes_semovientes_depreciadosd.php" method="get">
