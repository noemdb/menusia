<?php include ("../class/conect.php");  include ("../class/funciones.php");  $equipo=getenv("COMPUTERNAME");  $mcod_m="BIEN025".$usuario_sia.$equipo; if (!$_GET){$codigo_mov=substr($mcod_m,0,49);}else{$codigo_mov=$_GET["codigo_mov"];}
$conn=pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname."");if (pg_last_error($conn)) { echo "<p><b>Ocurrio un error conectando a la base de datos: .</b></p>"; exit; }
?>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">
<html>
<head>
<title>SIPAP CONTROL DE BIENES NACIONALES (Incluir Movimiento Bienes Muebles)</title>
...
function Llama_Eliminar(){var murl; var r;
 if (Gcodigo=="") {alert("Codigo del Mueble debe ser Seleccionada");}
  ...
  if(r==true){ r=confirm("Esta Realmente seguro en Eliminar el Bien del Movimiento ?");
    if(r==true){murl="Delete_cod_bienes_mov.php?codigo_mov="+Gcodigo_mov+"&codigo="+Gcodigo;document.location=murl;}
...
var murl;
  ...
  else{ murl="Mod_codigo_bienes_mue_movimientos.php?codigo_mov="+Gcodigo_mov+"&codigo="+Gcodigo; document.location=murl;}
}

function Llama_Cargar(codigo_mov){var murl;
   murl="cargar_bienes_mov.php?codigo_mov="+codigo_mov; document.location=murl;
}
</script>
<body>
 <table width="1745" border="0" cellspacing="0" cellpadding="0">
   <tr>
      <td align="left"><table width="840" border="0" align="left">
          <tr>
            <td width="210" align="center" valign="middle"><input name="btAgregar" type="button" id="btAgregar" value="Agregar" title="Agregar Codigo al Movimiento" onclick="javascript:LlamarURL('Inc_bien_movim.php?codigo_mov=<?php echo $codigo_mov?>')"></td>
   ...
   <tr>
     <td>
<?php  $sql="SELECT * FROM CODIGOS_BIEN050_MOV  where codigo_mov='$codigo_mov' order by cod_bien"; $res=pg_query($sql); ?>
       <table width="1740"  border="1" cellspacing='0' cellpadding='0' align="left" id="codigos">
         <tr height="20" class="Estilo5">
           ...
           <td width="550" align="center" bgcolor="#99CCFF"><strong>Denominaci�n</strong></td>
