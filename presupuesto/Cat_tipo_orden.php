...
<html>
<head>
<title>SIPAP CONTABILIDAD PRESUPUESTARIA (Catalogo Tipos de Orden)</title>
...
<?php 
        ...
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron Tipos de Orden</font>";$pagina=1; $inicio=1; $final=1; $numPags=1; 
              ...
              if ($_GET["orden"]==""){$orden="tipo_orden";}  else{$orden=$_GET["orden"];} $tamPag=10; if ($_GET["pagina"]==""){$pagina=1;$inicio=1;$final=$tamPag;}else{$pagina=$_GET["pagina"];} $limitInf=($pagina-1)*$tamPag; $numPags=ceil($numeroRegistros/$tamPag);
                ...
                $linea=0;   $Salir=false;
                while($registro=pg_fetch_array($res)){ $linea=$linea+1; $tasa_retencion=$registro["tasa"]; $tasa_retencion=formato_monto($tasa_retencion);
                if  ($linea>$limitInf+$tamPag){$Salir=true;}     if  (($linea>=$limitInf) and ($linea<=$limitInf+$tamPag)){?>
  <tr bgcolor='#FFFFFF' bordercolor='#000000' onMouseOver="this.style.backgroundColor='#CCCCCC';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#FFFFFF'"o"];" onDblClick="javascript:cerrar_catalogo('<?php  echo $registro["tipo_orden"]; ?>','<?php  echo $registro["des_tipo_orden"]; ?>','<?php  echo $registro["cod_contable_t"]; ?>');" >
    <td><font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#000033"><b><?php  echo $registro["tipo_orden"]; ?></b></font></td>
    <td><font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#000033"><b><?php  echo $registro["des_tipo_orden"]; ?></b></font></td>
    <td><font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#000033"><b><?php  echo $registro["cod_contable_t"]; ?></b></font></td>
     </tr>
<?php } }echo "</table>"; }?>
<br>
