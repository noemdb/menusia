...
function cerrar_catalogo(mcodigo,mfuente,mdenominacion,mdes_fuente,mcod_conta,mnom_cuenta,mdisponible,mdisp_diferida){
  //alert(mcodigo);	
...
<html>
<head>
<title>SIPAP CONTABILIDAD PRESUPUESTARIA (Catalogo de Codigos Presupuestarios)</title>
...
<?      $criterio=""; $txt_criterio=""; $pagina=1;$inicio=1;$final=1;
        ...
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron Codigos</font>"; $pagina=1; $inicio=1; $final=1; $numPags=1;
              ...
              if ($_GET["orden"]==""){$orden="cod_presup,cod_fuente";}else{$orden=$_GET["orden"];} $tamPag=10;
                ...
                $linea=0; $Salir=false;
                while($registro=pg_fetch_array($res)){ $denominacion=$registro["denominacion"]; $des_fuente_financ=substr($registro["des_fuente_financ"],0,100);
                $linea=$linea+1;  if($registro["disponible"]>0){$monto=formato_monto($registro["disponible"]);} else {$monto=0;} if($registro["disp_diferida"]>0){$montod=formato_monto($registro["disp_diferida"]);} else {$montod=0;}
				$l=strlen($denominacion); if($l>100){$l=100;}else{$l-1;}$denominacion=substr($denominacion,0,$l);
                if  ($linea>$limitInf+$tamPag){$Salir=true;}  if  (($linea>=$limitInf) and ($linea<=$limitInf+$tamPag)){  $denominacion=cambiar_car_especiales($denominacion);
?>
  <tr bgcolor='#FFFFFF' bordercolor='#000000' onMouseOver="this.style.backgroundColor='#CCCCCC';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#FFFFFF'"o"];"  onDblClick="javascript:cerrar_catalogo('<? echo $registro["cod_presup"]; ?>','<? echo $registro["cod_fuente"]; ?>','<? echo $denominacion; ?>','<? echo $des_fuente_financ; ?>','<? echo $registro["cod_contable"]; ?>','<? echo $registro["nombre_cuenta"]; ?>','<? echo $monto; ?>','<? echo $montod; ?>') ">
...
<form  name="form1" action="Cat_codigos_presup_comp.php" method="get" onSubmit="return revisar()">
<table border="0">
  <tr>
	<td>
