...
if (!$_GET){  $cod_presup="";$cod_fuente="00";  $mcod_m="PRE011".$equipo;$codigo_mov=substr($mcod_m,0,49);}
...
<html>
<head>
<title>SIPAP CONTABILIDAD PRESUPUESTARIA (Modificar Codigos en el Ajuste)</title>
...
function llamar_anterior(){document.location ='Det_inc_ajustes_comp.php?codigo_mov=<?echo $codigo_mov?>';}
...
$sql="SELECT * FROM CODIGOS_PRE026 where codigo_mov='$codigo_mov' and cod_presup='$cod_presup' and fuente_financ='$cod_fuente' and ref_imput_presu='$ref_imput_presu'";
$res=pg_query($sql);
if ($registro=pg_fetch_array($res,0)){  $denominacion=$registro["denominacion"];  $monto=$registro["monto"];  $des_fuente=$registro["des_fuente_financ"];
  $tipo_imput_presu=$registro["tipo_imput_presu"];  $montoc=$registro["monto_credito"];  $montocomp=$registro["monto_presup"];  $credito=$registro["amort_anticipo"];
  $cod_contable=$registro["cod_con_g_pagar"];  if($tipo_imput_presu=="P"){$tipo_imput_presu="PRESUPUESTO";}else{$tipo_imput_presu="CRED. ADICIONAL";}
...
?>
<body>
<form name="form1" method="post" action="Update_cod_ajust_comp.php" onSubmit="return revisar()">
  <table width="707" height="70" border="1" align="center" cellpadding="0" cellspacing="0">
    <tr>
      <td><table width="692" border="0" cellpadding="0" cellspacing="0">
        <tr>
          <td height="31" align="center" bgcolor="#000066"><span class="Estilo2 Estilo6">AJUSTAR MONTO DEL C&Oacute;DIGO</span></td>
        </tr>
        <tr>
        ...
        <tr>
          <td><table width="689" border="0">
            <tr>
              <td width="167"><span class="Estilo5">SALDO DEL COMPROMISO :</span></td>
              ...
              <td width="167"><span class="Estilo5"><input class="Estilo10" name="txtmonto" type="text" id="txtmonto" style="text-align:right" onFocus="encender_monto(this)" onBlur="apagar(this)" value="<? echo $monto ?>" onKeypress="return validarNum(event)" size="25" maxlength="22" > </span></td>
            </tr>
          </table></td>
        </tr>
        <tr>
