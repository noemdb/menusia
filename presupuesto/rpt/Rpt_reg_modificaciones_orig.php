<?php include ("../../class/conect.php");  include("../../class/fun_fechas.php"); include("../../class/fun_numeros.php"); include ("../../class/configura.inc"); $php_os=PHP_OS;
if (!$_GET){ $referencia_modif=''; $tipo_modif='';} else { $referencia_modif=$_GET["txtreferencia_modif"];$tipo_modif=$_GET["txttipo_modif"];}
  $sql="Select * FROM PRE009 where tipo_modif='$tipo_modif' and referencia_modif='$referencia_modif'";
  $rif_emp="";
$conn=pg_connect("host=".$host." port=".$port." password=".$password." user=".$user." dbname=".$dbname."");
if (pg_last_error($conn)) { echo "<p><b>Ocurrio un error conectando a la base de datos: .</b></p>"; exit; }  else { $Nom_Emp=busca_conf();  if($utf_rpt=="SI"){ $php_os="WINNT";}    }
...
if($filas>0){  $registro=pg_fetch_array($res);
  $referencia_modif=$registro["referencia_modif"];  $fecha_registro=$registro["fecha_registro"]; $ano_registro=substr($registro["fecha_registro"],0,4);
...
class PDF extends FPDF{ 
	...
	function Footer(){ global $total_comp; $ffechar=date("d-m-Y");$fhorar=date("H:i:s a");  $total_c=formato_monto($total_comp);
		...
		$this->Cell(66.6,6,'PRESUPUESTO',1,0,'C',true);
		$this->Cell(66.6,6,'ADMINISTRACION',1,0,'C',true);
		$this->Cell(66.7,6,'PRESIDENTE',1,1,'C',true);
		$z=$this->GetY();		
		$this->SetFont('Arial','',5);
		$this->Cell(200,19,' ',0,1,'C');
		$this->Cell(66.6,3,'',0,0,'C');	
        $this->Cell(66.6,3,'',0,0,'C');
        $this->Cell(66.8,3,'',0,1,'C');	
		$this->Cell(66.6,3,'',0,0,'C');	
        $this->Cell(66.6,3,'',0,0,'C');
        $this->Cell(66.8,3,'',0,1,'C'); 
		$x=$this->GetY()-0.5;
		$this->Line(76.6,$z,76.6,$x);	
		$this->Line(143.2,$z,143.2,$x);	
		$this->SetFillColor(255,0,0);
		$this->Ln(2);
		$this->SetFont('Arial','',5);
		$this->Cell(100,4,'SIPAP Contabilidad Presupuestaria',0,1,'L');		
	}
}	
$pdf=new PDF('P', 'mm', Letter);
  ...
  if($tipo_modif==5){ 
    $sql="SELECT * FROM codigos_modif where tipo_modif='$tipo_modif' and referencia_modif='$referencia_modif' and operacion='-' order by cod_presup,fuente_financ"; $res=pg_query($sql);$filas=pg_num_rows($res);if($filas>0){  $registro=pg_fetch_array($res);  $des_fuente_financ=$registro["des_fuente_financ"];}
    $pdf->SetFont('Arial','B',7);
	$pdf->Cell(40,4,'',0,0,'C');
	$pdf->Cell(140,4,'DE : ',0,0,'L');
	$pdf->Cell(20,4,'',0,1,'R');
	$pdf->SetFont('Arial','',7);
  ...
  }else{ $sql="SELECT * FROM codigos_modif where tipo_modif='$tipo_modif' and referencia_modif='$referencia_modif' order by cod_presup,fuente_financ"; $res=pg_query($sql);$filas=pg_num_rows($res);if($filas>0){  $registro=pg_fetch_array($res);  $des_fuente_financ=$registro["des_fuente_financ"];}
    $res=pg_query($sql);$filas=pg_num_rows($res); $total_mod=0; $i=0;  
    while($registro=pg_fetch_array($res)){ $i=$i+1;  
      $monto=formato_monto($registro["monto"]);  $denominacion=$registro["denominacion"];	
	  if($php_os=="WINNT"){$denominacion=$denominacion;}else{$denominacion=utf8_decode($denominacion);}	
