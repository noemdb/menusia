...
function cerrar_catalogo(mcodigo,mnombre){
  window.opener.document.forms[0].txttipo_ajusted.value = mcodigo;
  window.opener.document.forms[0].txttipo_ajusted.focus();
  window.close();
}
</script>
...
<html>
<head>
<title>SIPAP CONTABILIDAD PRESUPUESTARIA(Catalogo de Tipo de Ajuste)</title>
...
<?php 
        ...
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron resultados</font>"; $pagina=1; $inicio=1; $final=1; $numPags=1;
                ...
                echo "<th height='20' bgcolor='#99CCFF' ><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=tipo_ajuste&criterio=".$txt_criterio."'>Tipo</a></th>";
