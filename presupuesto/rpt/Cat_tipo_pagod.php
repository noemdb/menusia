...
function cerrar_catalogo(mcodigo,mnombre){
  window.opener.document.forms[0].txttipo_pagod.value = mcodigo;
  window.opener.document.forms[0].txttipo_pagod.focus();
  window.close();
}
</script>
...
<html>
<head>
<title>SIPAP CONTABILIDAD PRESUPUESTARIA(Catalogo de Tipo de Pagos)</title>
...
<?php 
        ...
        }else{ if ($_GET["orden"]==""){$orden="nombre_abrev_pago";}else{$orden=$_GET["orden"];} $tamPag=10;
...
<form action="Cat_tipo_pagod.php" method="get">
