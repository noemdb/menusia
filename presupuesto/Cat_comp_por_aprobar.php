...
<table width="978" height="38" border="0" bgcolor="#000066">
  <tr>
    ...
    <td width="836"><div align="center" class="Estilo2 Estilo6">CATALOGO COMPROMISOS POR APROBAR</div></td>
...
<?php       $criterio=""; $txt_criterio=""; $pagina=1;$inicio=1;$final=1;
        $criterio = " where (anulado<>'S' and aprobado<>'S' and tipo_compromiso<>'0000' and tipo_compromiso<>'A000') ";
        if ($_GET){if ($_GET["criterio"]!=""){$txt_criterio = $_GET["criterio"];$txt_criterio = strtoupper ($txt_criterio);
        $criterio = $criterio." and tipo_compromiso like '%" . $txt_criterio . "%' or referencia_comp like '%" . $txt_criterio . "%' or nro_documento like '%" . $txt_criterio. "%' or nombre like '%" . $txt_criterio . "%' or descripcion_comp like '%" . $txt_criterio . "%'";}}
        ...
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron Compromisos</font>";$pagina=1; $inicio=1; $final=1; $numPags=1;
                ...
                echo "<th bgcolor='#99CCFF'><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=descripcion_comp,tipo_compromiso,referencia_comp&criterio=".$txt_criterio."'>Descripci&oacute;n</a></th>";
                echo "<th bgcolor='#99CCFF'><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=aprobado,fecha_compromiso,referencia_comp,tipo_compromiso&criterio=".$txt_criterio."'>Status</a></th>";
                $linea=0;  $Salir=false;
                while($registro=pg_fetch_array($res)){ $linea=$linea+1; $descripcion=$registro["descripcion_comp"];  $sfecha=$registro["fecha_compromiso"];
				$anulado=$registro["anulado"]; $aprobado=$registro["aprobado"]; $msta="";  if($aprobado=='S'){$msta="APROBADO";} if($aprobado=='D'){$msta="DEVUELTA";}if($anulado=='S'){$msta="ANULADO";}
                $descripcion=substr($descripcion,0,150);$fecha=substr($sfecha,8,2)."/".substr($sfecha,5,2)."/".substr($sfecha,0,4);
                if  ($linea>$limitInf+$tamPag){$Salir=true;}
                if  (($linea>=$limitInf) and ($linea<=$limitInf+$tamPag)){
?>
  <tr bgcolor='#FFFFFF' bordercolor='#000000' onMouseOver="this.style.backgroundColor='#CCCCCC';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#FFFFFF'"o"];" onDblClick="javascript:CargarUrl('<?php  echo $registro["tipo_compromiso"].$registro["referencia_comp"].$registro["cod_comp"]; ?>');" >
	...
	<td><font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#000033"><b><?php  echo $msta; ?></b></font></td>
  </tr>
<?php } } echo "</table>"; }
?>
        <br>
...
<form action="Cat_comp_por_aprobar.php" method="get">
<table border="0" width="900">
  <tr>
	...
	<td width="100" >&nbsp;</td>
	<td width="100" onMouseOver="this.style.backgroundColor='#CCCCCC';this.style.cursor='hand';" onClick="javascript:LlamarURL('Act_compromisos.php')";
        onMouseOut="this.style.backgroundColor='#EAEAEA'"o"];" height="20" align="center" bgcolor=#EAEAEA><a class=menu href="Act_compromisos.php">Menu Anterior</a></div></td>
    <td width="100" >&nbsp;</td>
