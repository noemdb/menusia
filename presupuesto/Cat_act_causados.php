...
<html>
<head>
<title>SIPAP CONTABILIDAD PRESUPUESTARIA (Catalogo de Causados)</title>
<meta http-equiv="Content-type" content="text/html;charset=UTF-8">
<LINK href="../class/sia.css" type="text/css"  rel="stylesheet">
<script language="JavaScript" type="text/JavaScript">
function CargarUrl(mclave) {var murl;   murl="Act_causados.php?Gcriterio=C"+mclave;   document.location = murl;}
</script></head>
<body>
<table width="978" height="38" border="0" bgcolor="#000066">
  <tr>
    <td width="73"><div align="center" class="Estilo2 Estilo4"><img src="../imagenes/Logo_sia.gif" width="72" height="42"></div></td>
    <td width="836"><div align="center" class="Estilo2 Estilo6">CATALOGO CAUSADOS </div></td>
...
<?php 
        ...
        $criterio = " where tipo_causado like '%" . $txt_criterio . "%' or referencia_caus like '%" . $txt_criterio . "%' or descripcion_caus like '%" . $txt_criterio . "%'";}}
        $sql="SELECT * FROM CAUSADOS ".$criterio; $res=pg_query($sql);$numeroRegistros=pg_num_rows($res);
        if($numeroRegistros<=0){echo "<font face='verdana' size='-2'>No se encontraron Causados</font>"; $pagina=1; $inicio=1; $final=1; $numPags=1;
        }else{
              if(!isset($_GET["orden"])){$orden="tipo_causado,referencia_caus";} else{$orden=$_GET["orden"];} $tamPag=10;
                ...
                $sql="SELECT * FROM CAUSADOS ".$criterio." ORDER BY ".$orden; $res=pg_query($sql);
                ...
                echo "<th bgcolor='#99CCFF'><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=referencia_caus&criterio=".$txt_criterio."'>Referencia</a></th>";
                echo "<th bgcolor='#99CCFF'><a class='ord' href='".$_SERVER["PHP_SELF"]."?pagina=".$pagina."&orden=fecha_causado,referencia_caus&criterio=".$txt_criterio."'>Fecha</a></th>";
                ...
                $descripcion=$registro["descripcion_caus"]; $sfecha=$registro["fecha_causado"]; $descripcion=substr($descripcion,0,150);
                $fecha = substr($sfecha,8,2)."/".substr($sfecha,5,2)."/".substr($sfecha,0,4);
                if  ($linea>$limitInf+$tamPag){$Salir=true;}
                if  (($linea>=$limitInf) and ($linea<=$limitInf+$tamPag)){
?>
  <tr bgcolor='#FFFFFF' bordercolor='#000000' onMouseOver="this.style.backgroundColor='#CCCCCC';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#FFFFFF'"o"];" onDblClick="javascript:CargarUrl('<?php  echo $registro["tipo_causado"].$registro["referencia_caus"].$registro["tipo_compromiso"].$registro["referencia_comp"]; ?>');" >
    <td><font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#000033"><b><?php  echo $registro["tipo_causado"]; ?></b></font></td>
    <td><font size="2" face="Verdana, Arial, Helvetica, sans-serif" color="#000033"><b><?php  echo $registro["referencia_caus"]; ?></b></font></td>
...
<form action="Cat_act_causados.php" method="get">
<table border="0" width="900">
  <tr>
	...
	<td width="100" >&nbsp;</td>
	<td width="100" onMouseOver="this.style.backgroundColor='#CCCCCC';this.style.cursor='hand';" onClick="javascript:LlamarURL('Act_causados.php')";
        onMouseOut="this.style.backgroundColor='#EAEAEA'"o"];" height="20" align="center" bgcolor=#EAEAEA><a class=menu href="Act_causados.php">Menu Anterior</a></div></td>
    <td width="100" >&nbsp;</td>
