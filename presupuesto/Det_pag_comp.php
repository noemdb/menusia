...
<html>
<head>
<title>SIPAP CONTABILIDAD PRESUPUESTARIA (Detalles Pagos del Compromiso)</title>
...
?>
       <table width="768"  border="1" cellspacing='0' cellpadding='0' align="left" id="caus_comp">
         ...
         <?php  $total=0;$totaln=0;$totala=0;
while($registro=pg_fetch_array($res)){ $monto=formato_monto($registro["monto"]);   $ajustado=formato_monto($registro["ajustado"]);
  $neto=$registro["monto"]-$registro["ajustado"];  $totaln=$totaln+$neto;  $neto=formato_monto($neto);    $total=$total+$registro["monto"];  
  $totala=$totala+$registro["ajustado"];   $fecha=$registro["fecha_pago"];   if($fecha==""){$fecha="";}else{$fecha=formato_ddmmaaaa($fecha);}
?>
         <tr bgcolor='#FFFFFF' bordercolor='#000000' height="20" class="Estilo5" onMouseOver="this.style.backgroundColor='#CCCCCC';this.style.cursor='hand';" onMouseOut="this.style.backgroundColor='#FFFFFF'"o"];" >
           <td width="120" align="left"><?php  echo $registro["referencia_pago"].' '.$registro["tipo_pago"]; ?></td>
